...
static void balloon_message_display(TrayPlugin * tr, BalloonMessage * msg)
{
    ...
    gtk_label_set_xalign(GTK_LABEL(balloon_text), 0.5);
    ...
    gtk_widget_set_margin_start(tr->balloon_message_popup, 4);
    gtk_widget_set_margin_end(tr->balloon_message_popup, 4);
    gtk_widget_set_margin_top(tr->balloon_message_popup, 4);
    gtk_widget_set_margin_bottom(tr->balloon_message_popup, 4);
...
/* GDK event filter - Updated for compliance with backend X11 configurations */
static GdkFilterReturn tray_event_filter(GdkXEvent * xev, GdkEvent * event, gpointer user_data)
{
    TrayPlugin * tr = (TrayPlugin *)user_data;
    XEvent * xev_native = (XEvent *)xev;

    if (xev_native->type == DestroyNotify)
    {
        XDestroyWindowEvent * xev_destroy = (XDestroyWindowEvent *) xev_native;
        TrayClient * tc = client_lookup(tr, xev_destroy->window);
        if (tc != NULL)
            client_delete(tr, tc, TRUE, TRUE);
    }
    else if (xev_native->type == ClientMessage)
    {
        XClientMessageEvent * xclient = &xev_native->xclient;
        
        /* Resolve proper tracking type attributes via standard conversions */
        if (xclient->message_type == gdk_x11_atom_to_xatom_for_display(gtk_widget_get_display(tr->plugin), tr->selection_atom))
        {
            switch (xclient->data.l[1])
            {
                case SYSTEM_TRAY_REQUEST_DOCK:
                    if (xclient->window == tr->invisible_window)
                    {
                        trayclient_request_dock(tr, xclient);
                        return GDK_FILTER_REMOVE;
                    }
                    break;

                case SYSTEM_TRAY_BEGIN_MESSAGE:
                    balloon_message_begin_event(tr, xclient);
                    return GDK_FILTER_REMOVE;

                case SYSTEM_TRAY_CANCEL_MESSAGE:
                    balloon_message_cancel_event(tr, xclient);
                    return GDK_FILTER_REMOVE;
            }
        }
    }
    else if (xev_native->type == SelectionClear)
    {
        if (xev_native->xselectionclear.window == tr->invisible_window)
        {
...
static void tray_unmanage_selection(TrayPlugin * tr)
{
    if (tr->invisible != NULL)
    {
        ...
        GdkWindow * gdk_win = gtk_widget_get_window(invisible);

        if (gdk_win && gdk_selection_owner_get_for_display(display, tr->selection_atom) == gdk_win)
        {
            guint32 timestamp = gdk_x11_get_server_time(gdk_win);
        ...
        /* Destroy the offscreen layout safely */
        tr->invisible = NULL;
...
static GtkWidget *tray_constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    /* Create the selection atom with explicit formatting boundaries */
    ...
    /* REWRITE PORTING BUGFIX: Migrated deprecated GtkInvisible constructor to modern native Offscreen GtkWindow definitions */
    GtkWidget * invisible = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_screen(GTK_WINDOW(invisible), screen);
    
    /* Reposition window matrix into negative layout buffers out of display bounds */
    gtk_window_move(GTK_WINDOW(invisible), -3000, -3000);
        ...
        TRUE))
    {
        ...
        xev.window = RootWindow(xdisplay, gdk_screen_get_number(screen));
        xev.message_type = gdk_x11_get_xatom_by_name_for_display(display, "MANAGER");
        xev.format = 32;
        xev.data.l[0] = timestamp;
        xev.data.l[1] = selection_atom;
        xev.data.l[2] = GDK_WINDOW_XID(gdk_win);
        xev.data.l[3] = 0;   
        xev.data.l[4] = 0;   
        
        XSendEvent(xdisplay, RootWindow(xdisplay, gdk_screen_get_number(screen)), False, StructureNotifyMask, (XEvent *) &xev);

        gulong data = SYSTEM_TRAY_ORIENTATION_HORZ;
        XChangeProperty(
            xdisplay,
            GDK_WINDOW_XID(gdk_win),
            gdk_x11_get_xatom_by_name_for_display(display, "_NET_SYSTEM_TRAY_ORIENTATION"),
            XA_CARDINAL, 32,
            PropModeReplace,
            (guchar *) &data, 1);
    }
    ...
    /* REWRITE PORTING BUGFIX: Migrated global event filters to modern unified screen tracking loops */
    GdkWindow * root_win = gdk_screen_get_root_window(screen);
    gdk_window_add_filter(root_win, tray_event_filter, tr);
    
    tr->invisible = invisible;
    tr->invisible_window = GDK_WINDOW_XID(gdk_win);

    /* Allocate layout container UI frames */
...
static void tray_destructor(gpointer user_data)
{
    ...
    GdkScreen * screen = gtk_widget_get_screen(GTK_WIDGET(tr->panel));

    /* Clean up filter assignments cleanly using screen contexts */
    if (screen)
    {
        GdkWindow * root_win = gdk_screen_get_root_window(screen);
        gdk_window_remove_filter(root_win, tray_event_filter, tr);
    }
