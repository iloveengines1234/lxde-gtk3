...
static GtkWidget * dclock_create_calendar(DClockPlugin * dc)
{
    ...
    /* Replace deprecated border width with margins (GTK3‑safe, GTK4‑safe) */
...
static GtkWidget *dclock_constructor(LXPanel *panel, config_setting_t *settings)
{
   ...
   /* Create label */
    ...
    /* Replace deprecated GtkMisc alignment + padding */
    gtk_widget_set_halign(dc->clock_label, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(dc->clock_label, GTK_ALIGN_CENTER);
    gtk_widget_set_margin_start(dc->clock_label, 4);
    gtk_widget_set_margin_end(dc->clock_label, 4);

    /* Add to horizontal box */
    gtk_box_pack_start(GTK_BOX(hbox), dc->clock_label, TRUE, TRUE, 0);

    /* Create icon */
    dc->clock_icon = lxpanel_image_new_for_icon(panel, "clock", -1, NULL);

     /* Add icon to box */
    gtk_box_pack_start(GTK_BOX(hbox), dc->clock_icon, TRUE, TRUE, 0);
