    ...
    if (dm->folder_icon == NULL)
{
    ...
    /* Fallback to generic directory icon name */
    if (dm->folder_icon == NULL)
    {
        dm->folder_icon = gtk_icon_theme_load_icon(
            panel_get_icon_theme(dm->panel),
    ...
    {
    /* Create and initialize menu item (no deprecated image menu items). */
    GtkWidget *item = gtk_menu_item_new_with_label(dir_cursor->directory_name);

    /* Add folder icon manually */
    GtkWidget *img = gtk_image_new_from_icon_name("folder", GTK_ICON_SIZE_MENU);
    gtk_image_set_pixel_size(GTK_IMAGE(img), 16);  /* optional but looks better */
    gtk_menu_item_set_always_show_image(GTK_MENU_ITEM(item), TRUE);
    gtk_menu_item_set_image(GTK_MENU_ITEM(item), img);

    /* Create empty submenu placeholder */
    ...
    /* Add icon manually */
    GtkWidget *open_img = gtk_image_new_from_icon_name("document-open", GTK_ICON_SIZE_MENU);
    gtk_menu_item_set_always_show_image(GTK_MENU_ITEM(item), TRUE);
    gtk_menu_item_set_image(GTK_MENU_ITEM(item), open_img);

    g_signal_connect(item, "activate",
                    G_CALLBACK(dirmenu_menuitem_open_directory), dm);

    /* "Open in Terminal" menu item */
    GtkWidget *term = gtk_menu_item_new_with_mnemonic(_("Open in _Terminal"));

    /* Add terminal icon (optional but nice) */
    GtkWidget *term_img = gtk_image_new_from_icon_name("utilities-terminal", GTK_ICON_SIZE_MENU);
    gtk_menu_item_set_always_show_image(GTK_MENU_ITEM(term), TRUE);
    gtk_menu_item_set_image(GTK_MENU_ITEM(term), term_img);

    g_signal_connect(term, "activate",
                    G_CALLBACK(dirmenu_menuitem_open_in_terminal), dm);
...
static void dirmenu_show_menu(GtkWidget * widget, DirMenuPlugin * dm, int btn, guint32 time)
{
   ...
   /* Show the menu next to the top-level widget */
