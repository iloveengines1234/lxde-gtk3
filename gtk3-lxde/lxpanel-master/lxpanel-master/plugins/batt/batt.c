...
static gboolean exposeEvent(GtkWidget *widget, GdkEventExpose *event, gpointer user_data);
...
/* GTK3-safe timeout callback (called every 9 seconds) */
static gboolean update_timeout(lx_battery *lx_b)
{
    lx_b->state_elapsed_time++;
    lx_b->info_elapsed_time++;

    battery *bat = battery_update(lx_b->b);
...
{
    lx_battery *lx_b = (lx_battery *)user_data;
...
static gboolean
exposeEvent(GtkWidget *widget, GdkEventExpose *event, gpointer user_data)
{
    lx_battery *lx_b = (lx_battery *)user_data;
...
static GtkWidget * constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    gdk_rgba_parse(lx_b->backgroundColor, &lx_b->background);
    gdk_rgba_parse(lx_b->chargingColor1, &lx_b->charging1);
    gdk_rgba_parse(lx_b->chargingColor2, &lx_b->charging2);
    gdk_rgba_parse(lx_b->dischargingColor1, &lx_b->discharging1);
    gdk_rgba_parse(lx_b->dischargingColor2, &lx_b->discharging2);
