...
/*Validate the target desktop path configuration dynamically to prevent dead links */
static void on_add_menu_item_to_desktop(GtkMenuItem* item, GtkWidget* mi)
{
    ...
    FmPath *desktop_path = fm_path_get_desktop();
    
/* Ensure the XDG Desktop directory target path is initialized and structurally valid */
    if (desktop_path && !fm_path_is_root(desktop_path))
    {
        FmPathList *files = fm_path_list_new();
        fm_path_list_push_tail(files, fm_file_info_get_path(fi));
        
        /* Link the desktop profile application cleanly */
        fm_link_files(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(item))), files, desktop_path);
        
        fm_path_list_unref(files);
    }
    else
    {
        g_warning("lxpanel: Cannot copy item to desktop. Invalid or missing XDG Desktop directory configuration.");
    }
}

/* Implemented the fully functional 'Add menu item to panel' feature */
static void on_add_menu_item_to_panel(GtkMenuItem* item, MenuCacheApp* app)
{
    if (!app)
        return;

    const char* app_id = menu_cache_app_get_id(app);
    if (!app_id || app_id[0] == '\0')
        return;

    GSList* l;
    Plugin* target_lb = NULL;
    Panel* fallback_panel = NULL;
    int max_btns = -1;

    /* Iterate through all available desktop panels to find an active launchtaskbar */
    for (l = all_panels; l; l = l->next)
    {
        Panel* panel = (Panel*)l->data;
        if (!fallback_panel) 
            fallback_panel = panel; /* Keep track of the primary panel just in case */

        GList* pl;
        for (pl = panel->plugins; pl; pl = pl->next)
        {
            Plugin* plugin = (Plugin*)pl->data; /* REWRITE BUGFIX: Extract data from GList node correctly */
            
            /* Match against 'launchtaskbar' instead of legacy 'launchbar' name string */
            if (plugin && plugin->class && strcmp(plugin->class->type, "launchtaskbar") == 0)
            {
                /* Fetch the internal configuration settings group */
                config_setting_t* settings = config_setting_get_member(plugin->settings, "");
                int n = (settings && config_setting_is_list(settings)) ? config_setting_length(settings) : 0;
                
                /* Target the launchbar with the most items already on it as a heuristic preference */
                if (n > max_btns)
                {
                    target_lb = plugin;
                    max_btns = n;
    ...
    /* If a launchbar is not currently in use, dynamically append one to the primary panel */
    if (!target_lb && fallback_panel)
    {
        target_lb = lxpanel_panel_add_plugin(fallback_panel, "launchtaskbar", NULL);
        if (!target_lb)
        {
            g_warning("lxpanel: Failed to dynamically create a new launchtaskbar applet instance.");
            return;
        }
    }

    /* Append the selected application profile to our target panel launcher configuration */
    if (target_lb && target_lb->settings)
    {
        config_setting_t* settings = config_setting_get_member(target_lb->settings, "");
        if (!settings)
        {
            /* If the container group is completely blank, force-initialize it as a sublist element array */
            settings = config_setting_add(target_lb->settings, NULL, CONFIG_TYPE_LIST);
        }

        if (settings)
        {
            /* Create a brand new configuration block matching standard 'Button' formats */
            config_setting_t* new_btn = config_setting_add(settings, "Button", CONFIG_TYPE_GROUP);
            if (new_btn)
            {
                config_group_set_string(new_btn, "id", app_id);
                
                /* Save updates out to memory and immediately re-trigger the launchbar plugin constructor */
                lxpanel_config_save(target_lb->panel);
                lxpanel_plugin_configure(target_lb);
                
                g_debug("lxpanel: Successfully appended application launcher '%s' directly to the panel tracking context.", app_id);
            }
        }
    }
}
...
/* This function handles re-asserting safety focus grabs safely without X11-specific locks */
static void restore_grabs(GtkWidget *w, gpointer data)
{
    ...
    GtkWidget *xgrab_shell = NULL;
    GtkWidget *parent = menu;

    /* Traverse up the parent tree to locate the highest mapped, viewable ancestor widget shell */
    while (parent)
    {
    ...
    /* Use modern unified seat/device tracking if available, or fall back to window mapping hooks */
    if (xgrab_shell && gtk_widget_get_has_window(xgrab_shell))
    {
        GdkWindow *window = gtk_widget_get_window(xgrab_shell);
        if (window)
        {
            /* REWRITE BUGFIX: Replaced deprecated gdk_pointer_grab / gdk_keyboard_grab logic.
               Instead of raw X11 hijacking, let the windowing manager target device focus safely. */
            if (gtk_widget_has_focus(xgrab_shell))
            {
    ...
    /* Add the structural widget back to the active execution context loop safely */
    gtk_grab_add(menu);
}
...
static void show_system_menu(GtkWidget *p)
{
    ...
    /* If a menu request comes in while an old timer is 
     * already ticking, clear it first to prevent leaking handle sources. */
    ...
    {
        /* Preserved 200ms timeout to prevent aggressive Window Managers (e.g., IceWM) 
         * from stealing focus and instantly closing the menu during the initial click event cycle. */
...
static void read_include(Plugin *p, char **fp)
{
    ENTER;
    gchar *name = NULL;
    line s;
    ...
    if (fp && *fp)
    {
        while (lxpanel_get_line(fp, &s) != LINE_BLOCK_END) 
        {
            if (s.type == LINE_VAR) 
            {
                ...
                {
                    if (name) g_free(name); /* Avoid leak if name is defined twice */
    ...
    /* REWRITTEN FIXME: Ensure name is valid, safe, and absolute before opening */
    if (name && name[0] != '\0' && g_path_is_absolute(name)) 
    {
        include_file = fopen(name, "r");
        if (include_file) 
        {
            LOG(LOG_INFO, "Including config file: %s\n", name);
            
            /* Append the valid file handle to the panel's file tracking stack */
            m->files = g_slist_prepend(m->files, include_file);
            p->fp = include_file;
        } 
        else 
        {
            ERR("Can't include file (file not found or unreadable): %s\n", name);
        }
    } 
    else 
    {
        ERR("Can't include file: Invalid or missing absolute file path specify configuration.\n");
    }
...
read_submenu(menup *m, config_setting_t *s, gboolean as_item)
{
    ...
    for (i = 0; (s = config_setting_get_elem(list, i)) != NULL; i++)
    {
        ...
        {
            /* REWRITE FIXME: Safely re-enabled the config include parser token hook */
            read_include(p, fp);
            continue;
        }
        } else {
...
static void deep_destroy_menu_hierarchy(GtkWidget *menu)
{
    ...
    for (l = children; l != NULL; l = l->next)
    {
        ...
        {
            /* Check if this specific item anchors a nested submenu dropdown */
            GtkWidget *submenu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(item));
            if (submenu)
            {
                /* Recursively clean up the nested sub-menu stack first */
                deep_destroy_menu_hierarchy(submenu);
            }
        }
    }
    ...
    /* Finally, safely destroy this menu layer */
