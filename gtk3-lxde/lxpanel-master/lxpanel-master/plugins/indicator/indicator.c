           ...
           guint32 timestamp, gpointer user_data)
{
  ...
  if (entry == NULL) {
    ...
    GList * ent = NULL;
    for (ent = entries; ent != NULL; ent = g_list_next(ent)) {
      IndicatorObjectEntry * entrydata = (IndicatorObjectEntry *)ent->data;
  ...
  /* Show the menu for the given entry if present. Try to locate the
   * menuitem corresponding to this entry so the menu can be positioned
   * relative to it; otherwise just popup the menu. */
  if (entry->menu == NULL)
    return;
  ...
  if (array[1] != NULL) {
    GtkWidget *menuitem = GTK_WIDGET(array[1]);
    /* Popup the menu with the menuitem as parent for proper placement */
    gtk_menu_popup(GTK_MENU(entry->menu), NULL, NULL, NULL, menuitem, 0, timestamp);
  } else {
    /* Fallback: just popup the menu (positioning may depend on pointer) */
    gtk_menu_popup(GTK_MENU(entry->menu), NULL, NULL, NULL, NULL, 0, timestamp);
  }
}
...
load_module (const gchar * name, GtkWidget * menubar)
{
    ...
    if (io == NULL) {
        g_warning("Unable to create indicator object from %s", fullpath);
        g_free(fullpath);
        return FALSE;
    }
    g_free(fullpath);
...
static gboolean
menubar_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    GtkWidget *menubar = GTK_WIDGET(user_data);
...
static void indicator_load_modules(LXPanel *panel, GtkWidget *p)
{
    ...
    if (g_file_test(INDICATOR_DIR, (G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)))
    {
        ...
        if (dir == NULL) {
            g_warning("Unable to open indicator directory: %s", INDICATOR_DIR);
        } else {
            const gchar *name;
...
static GtkWidget *indicator_constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    g_signal_connect_after(indicator->menubar, "draw", G_CALLBACK(menubar_draw), indicator->menubar);
