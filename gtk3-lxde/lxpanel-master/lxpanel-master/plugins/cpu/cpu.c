...
/*Entire component of cpu.c ported to gtk3.24.52 by iloveengines1234 on GitHub*/
...
typedef struct {
    ...
    guint pixmap_height;              /* Pixmap to be drawn on drawing area pixmap; does not include border size */
    struct cpu_stat previous_cpu_stat;/* Previous value of cpu_stat */
    gboolean initialized;             /* Whether previous_cpu_stat was initialized */
} CPUPlugin;

static void redraw_pixmap(CPUPlugin *c);
static gboolean cpu_update(gpointer user_data);
...
static void redraw_pixmap(CPUPlugin *c)
{
    if (!c->pixmap || !c->stats_cpu)
        return;

    cairo_t *cr = cairo_create(c->pixmap);

    /* Erase pixmap with black background. */
    ...
    for (i = 0; i < c->pixmap_width; i++)
    {
        ...
        if (c->stats_cpu[drawing_cursor] != 0.0f)
        {
            cairo_move_to(cr, i + 0.5, c->pixmap_height);
            cairo_line_to(cr, i + 0.5,
                          c->pixmap_height - c->stats_cpu[drawing_cursor] * c->pixmap_height);
            cairo_stroke(cr);
        }
...
static gboolean cpu_update(gpointer user_data)
{
    ...
    if ((c->stats_cpu != NULL) && (c->pixmap != NULL))
    {
        ...
        if (fscanf_result == 4)
        {
            /* On the first successful read initialize previous_cpu_stat and skip introducing a sample
             * to avoid a spike caused by comparing against zero-initialized previous values. */
            if (!c->initialized)
            {
            ...
            float cpu_uns = (float)(cpu_delta.u + cpu_delta.n + cpu_delta.s);
            c->stats_cpu[c->ring_cursor] = cpu_uns / (cpu_uns + (float)cpu_delta.i);
            c->ring_cursor += 1;
...
static gboolean configure_event(GtkWidget *widget, GdkEventConfigure *event, CPUPlugin *c)
{
    ...
    if ((new_pixmap_width > 0) && (new_pixmap_height > 0))
    {
            ...
            if (c->stats_cpu != NULL)
            {
                if (new_pixmap_width > c->pixmap_width)
                {
                    ...
                    memcpy(&new_stats_cpu[new_pixmap_width - c->pixmap_width + c->ring_cursor],
                           ...
                           (c->pixmap_width - c->ring_cursor) * sizeof(CPUSample));
                }
                else if (c->ring_cursor <= new_pixmap_width)
                {
                    ...
                    memcpy(&new_stats_cpu[c->ring_cursor],
                           ...
                           (new_pixmap_width - c->ring_cursor) * sizeof(CPUSample));
                }
                else
                {
                    ...
                    memcpy(&new_stats_cpu[0],
                           ...
                           new_pixmap_width * sizeof(CPUSample));
                    c->ring_cursor = 0;
                }
                g_free(c->stats_cpu);
            }
        ...
        c->pixmap = cairo_image_surface_create(CAIRO_FORMAT_RGB24,
                                               c->pixmap_width, c->pixmap_height);
...
static GtkWidget *cpu_constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    /* Set foreground color for the graph. */
    gdk_rgba_parse(&c->foreground_color, "green");

    /* Connect signals. */
    g_signal_connect(c->da, "configure-event",
                     G_CALLBACK(configure_event), c);
    g_signal_connect(c->da, "draw",
                     G_CALLBACK(draw), c);

    /* Show the widget.  Connect a timer to refresh the statistics. */
    gtk_widget_show(c->da);
    c->timer = g_timeout_add(1500, (GSourceFunc) cpu_update, c);
