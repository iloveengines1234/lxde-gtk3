...
static gboolean on_button_press (GtkWidget* widget, GdkEventButton* evt, LXPanel* p)
{
	...
	if (skip_button1_event) {
		gtk_widget_destroy(vol->dlg);
		vol->dlg = NULL;
		skip_button1_event = FALSE;
	}
	...
	case 1: {	/*  Left click */
		if ( ! vol->dlg ) {
			...
			/* GTK 3 applies default style through GtkStyleContext automatically */
...
static GtkWidget *volume_constructor(LXPanel *panel, config_setting_t *settings)
{
	...
	dialog = create_volume_window(); /* use pointer */
	if (! vol_spin)
		goto _error;
...
_error:
	...
	vol->dlg = dialog; /* it was reused */

	event_box = gtk_event_box_new();
	lxpanel_plugin_set_data(event_box, vol, volume_destructor);
	vol->panel = panel;

	g_signal_connect(event_box, "scroll-event", G_CALLBACK(on_mouse_scroll), vol);
	gtk_widget_set_size_request(event_box, panel_get_icon_size(panel), panel_get_icon_size(panel));
	update_icon(p, vol);
	/* Clear the global spin pointer so we don't keep a stale reference
	 * to widgets owned by the temporary dialog that we're about to destroy. */
	vol_spin = NULL;
	gtk_widget_destroy( vol->dlg );
	vol->dlg = NULL;
	vol->dlg = NULL;
	...
	gtk_widget_set_tooltip_text(event_box, _("Volume control"));
