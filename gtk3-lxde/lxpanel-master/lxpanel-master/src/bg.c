...
/* Return a cairo_surface_t containing the root background as seen under widget */
cairo_surface_t *
fb_bg_get_xroot_pix_for_win (FbBg *bg, GtkWidget *widget)
{
    ...
    win = gdk_x11_window_get_xid (gtk_widget_get_window (widget));

    if (!XGetGeometry (bg->dpy, win, &dummy, &x, &y,
    ...
    DBG ("win=%lx %dx%d%+d%+d\n", (unsigned long)win, width, height, x, y);

    /* Create an X pixmap and fill it using the tiled GC */
    bgpix = XCreatePixmap (bg->dpy, bg->xroot, width, height, depth);
    if (bgpix == None) {
        g_critical ("XCreatePixmap failed");
        RET (NULL);
    }
    ...
    /* Wrap the X pixmap in a cairo surface */
    surface = cairo_xlib_surface_create (bg->dpy,
                                         bgpix,
                                         DefaultVisual (bg->dpy,
                                                        DefaultScreen (bg->dpy)),
                                         width,
                                         height);
...
void
fb_bg_composite (cairo_surface_t *base, GdkRGBA *tintcolor, gint alpha)
{
    cairo_t *cr;
    FbBg *bg;
    ...
    gdk_cairo_set_source_rgba (cr, tintcolor);
...
inline void
fb_bg_notify_changed_bg (FbBg *bg)
{
    ENTER;
...
/* Load an image file and return it as a cairo_surface_t */
cairo_surface_t *
fb_bg_get_pix_from_file (GtkWidget *widget, const char *filename)
{
    ...
    if (!pixbuf) {
        /* No GtkStyle pixmap fallback in GTK3; just return NULL */
        RET (NULL);
    }
