...
fb_ev_emit(FbEv *ev, int signal)
{
    ...
    if( signal == EV_ACTIVE_WINDOW )
    {
        Window* win = None;
        ev->active_window = None;
        /* FIXED: Swapped GDK_ROOT_WINDOW() out for screen context resolution */
        win = (Window*)get_xaproperty (gdk_screen_get_root_window(gdk_screen_get_default()), a_NET_ACTIVE_WINDOW, XA_WINDOW, 0);
        if (win) {
...
fb_ev_current_desktop(FbEv *ev)
{
    ...
    if (ev->current_desktop == -1) {
        ...
        /* FIXED: Swapped GDK_ROOT_WINDOW() out for screen context resolution */
        data = get_xaproperty (gdk_screen_get_root_window(gdk_screen_get_default()), a_NET_CURRENT_DESKTOP, XA_CARDINAL, 0);
        if (data) {
...
fb_ev_number_of_desktops(FbEv *ev)
{
    ENTER;
     if (ev->number_of_desktops == -1) {
        ...
        /* FIXED: Swapped GDK_ROOT_WINDOW() out for screen context resolution */
        data = get_xaproperty (gdk_screen_get_root_window(gdk_screen_get_default()), a_NET_NUMBER_OF_DESKTOPS, XA_CARDINAL, 0);
        if (data) {
