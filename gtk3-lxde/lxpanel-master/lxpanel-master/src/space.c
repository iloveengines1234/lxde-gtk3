...
/* Forward declaration for the local resize handler */
void _panel_space_resize(GtkWidget *widget, int size);

static gboolean
panel_space_drag_drop(GtkWidget *widget,
...
static gboolean
panel_space_make_launcher(GtkWidget *widget,
                          gint x,
                          gint y,
                          const char *str)
{
    ...
    if (!panel_box || !GTK_IS_CONTAINER(panel_box))
        return FALSE;

    gtk_container_child_get(GTK_CONTAINER(panel_box), widget,
                            "position", &idx,
                            "expand", &expand,
                            NULL);
    if (idx < 1)
    ...
    if (x <= icon_size / 2 + 4) /* leave launchbar at idx (before PS) */
    {
        lxpanel_config_save(panel);
    }
    else if (x >= size - icon_size / 2 - 4) /* move launchbar to idx+1 (after PS) */
    {
        gtk_box_reorder_child(GTK_BOX(panel_box), launchbar, idx + 1);
        config_setting_move_elem(cfg, config_setting_get_parent(cfg), idx + 2);
        lxpanel_config_save(panel);
    }
    else if (expand && x < size / 2) /* create another PS at idx */
    {
        cfg = config_group_add_subgroup(config_root_setting(panel->priv->config),
                                        "Plugin");
    ...
    else /* move launchbar to idx+1, then create another PS at idx+2 */
    {
        gtk_box_reorder_child(GTK_BOX(panel_box), launchbar, idx + 1);
        config_setting_move_elem(cfg, config_setting_get_parent(cfg), idx + 2);
        cfg = config_group_add_subgroup(config_root_setting(panel->priv->config),
    ...
    if (!expand) /* resize to sp->size - icon_size - 3 - size2 */
    {
...
static void
panel_space_drag_data_received(GtkWidget *widget,
                               GdkDragContext *context,
                               gint x,
                               gint y,
                               ...
                               guint time)
{
    ...
    case LAUNCHER_DND_TARGET:
        ...
        if (str == NULL || gtk_selection_data_get_length(sel_data) <= 0)
            break;
        ...
        break;
    default:
        fm_dnd_dest_drag_data_received(PANEL_SPACE(widget)->dd,
                                       context, x, y,
                                       sel_data, info, time);
    }
}
...
static gboolean
panel_space_drag_motion(GtkWidget *widget,
                        GdkDragContext *context,
                        gint x,
                        gint y,
                        guint time)
...
static void
panel_space_drag_leave(GtkWidget *widget,
...
static gboolean
panel_space_files_dropped(FmDndDest *dd,
                          int x,
                          int y,
...
static void
panel_space_get_preferred_width(GtkWidget *widget,
                                gint *minimal_width,
                                gint *natural_width)
{
...
static void
panel_space_get_preferred_height(GtkWidget *widget,
                                 gint *minimal_height,
                                 gint *natural_height)
{
    PanelSpace *p = PANEL_SPACE(widget);
    ...
    if (minimal_height)
        *minimal_height = 2;
    if (natural_height)
        *natural_height = p->size;
}

G_DEFINE_TYPE(PanelSpace, panel_space, GTK_TYPE_EVENT_BOX)

static void
panel_space_dispose(GObject *object)
{
    ...
    if (self->dd)
    {
        g_signal_handlers_disconnect_by_func(self->dd,
                                             G_CALLBACK(panel_space_files_dropped),
                                             self);
...
static void
panel_space_class_init(PanelSpaceClass *klass)
{
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    widget_class->get_preferred_width = panel_space_get_preferred_width;
    widget_class->get_preferred_height = panel_space_get_preferred_height;
    ...
    launcher_dnd_atom =
        gdk_atom_intern_static_string("application/x-lxpanel-launcher");
}

static void
panel_space_init(PanelSpace *self)
{
    gtk_widget_set_has_window(GTK_WIDGET(self), FALSE);

    self->dd = fm_dnd_dest_new(GTK_WIDGET(self));
    fm_dnd_dest_add_targets(GTK_WIDGET(self),
                            dnd_targets,
...
/* Plugin constructor. */
GtkWidget *
_panel_space_new(LXPanel *panel, config_setting_t *settings)
{
    /* Allocate plugin context and set into Plugin private data pointer. */
    PanelSpace *p = g_object_new(panel_space_get_type(), NULL);
...
static gboolean
space_apply_configuration(gpointer user_data)
{
    PanelSpace *p = (PanelSpace *) user_data;

    /* Apply settings. Forces layout engine to re-query preferred size. */
...
void
_panel_space_resize(GtkWidget *spacer, gint size)
{
...
gint
_panel_space_get_size(GtkWidget *spacer)
{
...
static GtkWidget *
space_configure(LXPanel *panel, GtkWidget *instance)
{
    ...
    dlg = lxpanel_generic_config_dlg(_("Spacer"), panel,
                                     ...
                                     C_("Width", "Size"), &p->size,
                                     CONF_TYPE_INT, NULL);

    gtk_widget_set_size_request(dlg, 200, -1);  /* Improve geometry setup */
