...
static void set_width_type( GtkWidget *item, LXPanel* panel )
{
    ...
    case WIDTH_PIXEL:
        {
            GdkDisplay *display = gtk_widget_get_display(spin);
            ...
            {
                gtk_spin_button_set_range( GTK_SPIN_BUTTON(spin), 0, geometry.width );
                gtk_spin_button_set_value( GTK_SPIN_BUTTON(spin), geometry.width );
            }
            else
            {
                gtk_spin_button_set_range( GTK_SPIN_BUTTON(spin), 0, geometry.height );
                gtk_spin_button_set_value( GTK_SPIN_BUTTON(spin), geometry.height );
            }
        }
        break;
    case WIDTH_REQUEST:
...
on_font_color_set(GtkColorButton* clr, LXPanel* panel)
{
    ...
    gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(clr), &p->gfontcolor);
    panel_set_panel_configuration_changed(p);
    /* Update your macro gcolor2rgb24 to handle components (0.0 to 1.0) of GdkRGBA */
    p->fontcolor = (((int)(p->gfontcolor.red * 255)) << 16) | 
                   (((int)(p->gfontcolor.green * 255)) << 8) | 
                   ((int)(p->gfontcolor.blue * 255));
    UPDATE_GLOBAL_RGBA(p, "fontcolor", p->fontcolor);
...
on_plugin_expand_toggled(GtkCellRendererToggle* render, char* path, GtkTreeView* view)
{
    ...
    if( gtk_tree_model_get_iter( model, &it, tp ) )
    {
        ...
        if (init->expand_available)
        {
            ...
            /* Query and change layout parameters natively through GtkContainer properties */
            gtk_container_child_set( GTK_CONTAINER(box), pl, "expand", expand, NULL );
...
static void on_add_plugin( GtkButton* btn, GtkTreeView* _view )
{
    ...
    parent_win = gtk_widget_get_ancestor( GTK_WIDGET(_view), GTK_TYPE_WINDOW );
...
void panel_configure( LXPanel* panel, int sel_page )
{
    ...
    else if( p->widthtype == WIDTH_PIXEL)
    {
        ...
        GdkRectangle geometry;
        gdk_monitor_get_geometry(monitor, &geometry);
        upper = (((p->edge == EDGE_TOP) || (p->edge == EDGE_BOTTOM)) ? geometry.width : geometry.height);
    }
    ...
    */
    ...
    /* Ensure p->gtintcolor is treated as GdkRGBA and its alpha property assigned properly */
    p->gtintcolor.alpha = (double)p->alpha / 255.0; 
    ...
    /* background */
    {
        ...
        else if ((info = gtk_icon_theme_lookup_icon(p->icon_theme, "lxpanel-background", 48, 0)))
        {
...
static void on_fc_dialog_response(GtkWidget *dialog, gint response_id, gpointer user_data)
{
    GtkButton *btn = GTK_BUTTON(user_data);
    GtkEntry *entry = GTK_WIDGET(g_object_get_data(G_OBJECT(btn), "target-entry"));
                                              ...
                                              const char *name, va_list args)
{
    GtkWidget* dlg = gtk_dialog_new_with_buttons( title, NULL, 0,
                                                  ...
                                                  GTK_RESPONSE_CLOSE,
                                                  NULL );
    /* In GTK3, the content area is a standard GtkBox with vertical orientation by default */
    ...
    while( name )
    {
        ...
        if( entry )
        {
            if(( type == CONF_TYPE_BOOL ) || ( type == CONF_TYPE_TRIM ))
                ...
                {
                /* FIXED: Replaced legacy gtk_hbox_new with modern oriented gtk_box_new */
