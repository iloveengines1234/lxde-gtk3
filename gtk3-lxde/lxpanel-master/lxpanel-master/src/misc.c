...
int get_net_number_of_desktops()
{
    ...
    ENTER;
    data = get_xaproperty (GDK_WINDOW_XID(root), a_NET_NUMBER_OF_DESKTOPS,
          XA_CARDINAL, 0);
    if (!data)
        RET(0);
...
int get_net_current_desktop ()
{
    ...
    ENTER;
    data = get_xaproperty (GDK_WINDOW_XID(root), a_NET_CURRENT_DESKTOP, XA_CARDINAL, 0);
    if (!data)
        RET(0);
...
static void calculate_width(int scrw, int pw, int wtype, int align, int margin, int *panw, int *x)
{
    ENTER;
    DBG("scrw=%d\n", scrw);
    ...
    if (align != ALIGN_CENTER) {
        if (margin > scrw) {
            g_warning( "margin is bigger then edge size %d > %d. Ignoring margin", margin, scrw);
            margin = 0;
        }
...
void _calculate_position(LXPanel *panel, GdkRectangle *rect)
{
    ...
    if (np->monitor < 0) {
        /* All monitors combined layout / fallbacks */
        monitor = gdk_display_get_monitor(display, 0);
        if (monitor) {
            gdk_monitor_get_geometry(monitor, &marea);
            /* Approximate spanning logic for legacy single root space */
            marea.x = 0;
            marea.y = 0;
            marea.width = gdk_monitor_get_geometry(monitor, &marea), marea.width; 
        } else {
            marea.x = marea.y = marea.width = marea.height = 0;
        }
    } else if (np->monitor < gdk_display_get_n_monitors(display)) {
        monitor = gdk_display_get_monitor(display, np->monitor);
        gdk_monitor_get_geometry(monitor, &marea);
    } else {
        marea.x = marea.y = marea.width = marea.height = 0;
    }
...
static void _lxpanel_button_set_icon(GtkWidget* btn, FmIcon* icon, gint size)
{
    ...
    if (img != NULL)
    {
        ...
        if (size == 0)
            size = data->size;
        if (icon != data->icon || size != data->size)
        {
            g_object_unref(data->icon);
            data->icon = icon;
            data->size = size;
...
static void _gtk_image_set_from_file_scaled(GtkWidget * img, ImgData * data)
{
    ...
    {
        /* Fixed Deprecation: Stock items removed in later GTK 3 versions */
...
get_button_spacing(GtkRequisition *req, GtkContainer *parent, gchar *name)
{
    ...
    /* GTK3 replacement for size request */
...
/* Updated to process GTK3 GdkRGBA instead of legacy GdkColor */
guint32 gcolor2rgb24(GdkRGBA *color)
{
    ...
    i = ((guint32)(color->red * 255.0)) & 0xFF;
    i <<= 8;
    i |= ((guint32)(color->green * 255.0)) & 0xFF;
    i <<= 8;
    i |= ((guint32)(color->blue * 255.0)) & 0xFF;
    DBG("i=%x\n", i);
    RET(i);
}
                                          ...
                                          gulong highlight_color, const gchar *label)
{
    ...
    /* GTK3 replacement for GtkMisc padding & alignment adjustments */
    ...
    else
    {
        ...
        /* GTK3 replacement for gtk_hbox_new */
        ...
        /* GTK3 Replacement for text padding */
