    ...
    if (w != NULL)
    {
        if (gtk_widget_get_has_window(w))
        {
            ...
            if (gtk_widget_get_realized(w))
            {
                ...
                {
                    /* In GTK3, GtkStyle background rendering is deprecated.
                     * We clear custom style provider overrides to default to current theme CSS. */
...
static gboolean lxpanel_plugin_button_press_event(GtkWidget *plugin, GdkEventButton *event, LXPanel *panel)
{
        ...
        (event->state & gtk_accelerator_get_default_mod_mask()) == 0) /* no key */
    {
        ...
        /* Modernized fallback signature mapping cleanly for GTK3 windowing captures */
...
void lxpanel_plugin_popup_set_position_helper(LXPanel * p, GtkWidget * near, GtkWidget * popup, gint * px, gint * py)
{
    ...
    if (gtk_widget_is_toplevel(popup))
    {
        ...
        GdkWindow *win = gtk_widget_get_window(popup);
        gdk_window_get_frame_extents(win, &extents);
    ...
    /* Push onscreen using modern display and monitor logic */
    display = gtk_widget_get_display(near);
    ...
    if (monitor)
    {
        ...
        x = CLAMP(x, monitor_geometry.x, monitor_geometry.x + monitor_geometry.width - popup_req.width);
        y = CLAMP(y, monitor_geometry.y, monitor_geometry.y + monitor_geometry.height - popup_req.height);
    }
