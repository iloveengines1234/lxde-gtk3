...
static gboolean open_search_func(GAppLaunchContext* ctx, GList* folder_infos, gpointer user_data, GError** err)
{
    ...
    for(; l; l=l->next)
    {
        /* Additional windows intentionally not opened in demo mode. */
    }
    return TRUE;
}
...
static void on_sel_changed(FmFolderView* fv, gint n_files, FmMainWin* win)
{
    ...
    {
        char* msg;
        /* Total size for multi-selection intentionally omitted: recursive size
        * calculation is expensive and libfm has no async folder-size API. */
...
static void create_bookmarks_menu(FmMainWin* win)
{
    ...
    for(l=items;l;l=l->next)
    {
        ...
        gtk_widget_show(mi);
        /* GTK3 hides menu icons and GTK4 deletes them entirely, so nope gng */
...
static void on_history_item(GtkMenuItem* mi, FmMainWin* win)
{
    ...
    /* Navigation is intentionally explicit; FmNavHistory does not emit signals. */
...
static void fm_main_win_init(FmMainWin *win)
{
    ...
    /* Load UI preferences */
        FmConfig *cfg = fm_config_get();

        /* Optional toolbar icon size */
        if (cfg->toolbar_icon_size == FM_TOOLBAR_ICON_SMALL)
            gtk_toolbar_set_icon_size(GTK_TOOLBAR(win->toolbar), GTK_ICON_SIZE_SMALL_TOOLBAR);
        else if (cfg->toolbar_icon_size == FM_TOOLBAR_ICON_LARGE)
            gtk_toolbar_set_icon_size(GTK_TOOLBAR(win->toolbar), GTK_ICON_SIZE_LARGE_TOOLBAR);
        /* Optional toolbar style (deprecated in GTK3 but still works) */
        if (cfg->toolbar_style == FM_TOOLBAR_STYLE_ICONS)
            gtk_toolbar_set_style(GTK_TOOLBAR(win->toolbar), GTK_TOOLBAR_ICONS);
        else if (cfg->toolbar_style == FM_TOOLBAR_STYLE_TEXT)
            gtk_toolbar_set_style(GTK_TOOLBAR(win->toolbar), GTK_TOOLBAR_TEXT);
        else if (cfg->toolbar_style == FM_TOOLBAR_STYLE_BOTH)
...
void on_rename(GtkAction* act, FmMainWin* win)
{
    ...
    if(files)
        ...
        /* Yes, it is ok to only rename the first selected file here. */
}
