...
static inline void _update_file_ptr_part(FmXmlFile *file,
                                         const char *start,
                                         const char *end)
{
    while (start < end)
    {
        ...
        {
            file->pos++;  /* advance by characters, not bytes */
        }
...
{
    const char *p = file->data->str;
    const char *end = p + file->data->len;
    ...
    while (p < end)
    {
        ...
        p = g_utf8_next_char(p);  /* move to next UTF-8 character */
    }
...
_restart:
    ...
    if (file->data && file->data->len && file->data->str[0] == '<')
    {
        ...
        /* we got a complete tag, nice, let's parse it */
