...
typedef struct
{
    ...
    gulong cancel_handler_id; /* Added track tracking ID to avoid dangling signal hooks */
} AsyncData;
...
static GIcon* g_udisks_volume_get_icon (GVolume* base)
{
    ...
    if(!vol->icon)
    {
        ...
        /* Modernized call to strip deprecated default fallbacks engine */
        vol->icon = g_themed_icon_new_from_names((char**)&icon_name, 1);
    }
...
static void mount_callback(DBusGProxy *proxy, char * OUT_mount_path, GError *error, gpointer user_data)
{
    ...
    /* Disconnect cancel signal tracking cleanly */
    if (data->cancellable && data->cancel_handler_id > 0)
    {
        g_signal_handler_disconnect(data->cancellable, data->cancel_handler_id);
        data->cancel_handler_id = 0;
    }
    ...
    else
    {
        ...
        /* Completely rewritten pointer allocation logic to eliminate memory smash vulnerability */
        if(data->vol->dev->mount_paths)
        {
            ...
            if(!*p) 
            {
                ...
                char** new_paths = g_new0(char*, len + 2);
                new_paths[0] = g_strdup(OUT_mount_path);
                for (int i = 0; i < len; i++)
                {
                    new_paths[i + 1] = data->vol->dev->mount_paths[i];
                }
                g_free(data->vol->dev->mount_paths);
                data->vol->dev->mount_paths = new_paths;
            }
        }
        else
        {
...
static void g_udisks_volume_mount_fn(GVolume* base, GMountMountFlags flags, GMountOperation* mount_operation, GCancellable* cancellable, GAsyncReadyCallback callback, void* user_data)
{
    ...
    if(cancellable)
    {
        ...
        data->cancel_handler_id = g_signal_connect(cancellable, "cancelled", G_CALLBACK(on_mount_cancelled), data);
    }
}
