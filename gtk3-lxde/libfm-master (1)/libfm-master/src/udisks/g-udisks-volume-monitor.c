...
/* Hooks into hardware eject signaling rules to dispatch up the stack safely */
void drive_eject_button(GVolumeMonitor *mon, GDrive *drive)
{
    GUDisksVolumeMonitor* umon = G_UDISKS_VOLUME_MONITOR(mon);
    
    if (G_IS_UDISKS_DRIVE(drive))
    {
        GUDisksDrive *drv = G_UDISKS_DRIVE(drive);
        
        /* 1. Fire the monitor-wide notification channel */
        g_signal_emit(umon, sig_drive_eject_button, 0, drv);
        
        /* 2. Dispatch the callback button trigger hook implemented in drive.c */
        g_udisks_drive_eject_button(drv);
    }
}
