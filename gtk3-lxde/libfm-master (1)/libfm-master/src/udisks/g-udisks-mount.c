...
typedef struct
{
    ...
    DBusGProxy* proxy;
    DBusGProxyCall* call;
    gulong cancel_handler_id;    /* Tracks the signal connection to prevent dangling handles */
    GCancellable* cancellable;   /* Keeps a temporary reference to safely disconnect on completion */
} AsyncData;
...
/* Modernized return type signature to gchar** to prevent strict interface linkage breaks */
static gchar** g_udisks_mount_guess_content_type_finish (GMount* base, GAsyncResult* res, GError** error)
{
...
static void unmount_callback(DBusGProxy *proxy, GError *error, gpointer user_data)
{
    ...
    /* Safely clear out the signal tracking engine if the hook hasn't fired yet */
...
static void g_udisks_mount_unmount_with_operation (GMount* base, GMountUnmountFlags flags, GMountOperation* mount_operation, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if(mnt->vol)
    {
        ...
        if(cancellable)
        {
            data->cancel_handler_id = g_signal_connect(cancellable, "cancelled", 
                                                       G_CALLBACK(on_unmount_cancelled), data);
        }
    }
}
