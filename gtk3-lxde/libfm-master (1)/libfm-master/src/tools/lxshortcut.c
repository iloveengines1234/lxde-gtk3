...
#define HIDE_WIDGET(x) do { \
    widget = gtk_builder_get_object(ui, x); \
    if (widget) \
...
int main(int argc, char **argv)
{
    ...
    /* setup LibFM for its default config safely */
    config = fm_config_new();
    fm_config_load_from_file(config, "/dev/null");
    fm_gtk_init(config);
    
    /* Access defaults dynamically to ensure compatibility with modernized libfm runtimes */
    FmConfig *default_cfg = fm_config_get_default();
    if (default_cfg) {
        g_strfreev(default_cfg->modules_blacklist);
        g_strfreev(default_cfg->modules_whitelist);
        default_cfg->modules_blacklist = g_strdupv(bl);
        default_cfg->modules_whitelist = g_strdupv(wl);
    }
    ...
    /* we have existing target file now with actual contents, let's edit it */
