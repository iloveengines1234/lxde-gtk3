...
struct _FmFileInfo
{
    ...
    /* Collation keys are cached on demand to speed up sorting.
    * This increases memory usage slightly but avoids recomputing
    * expensive UTF-8 collation keys repeatedly.
     */
...
if (path_str)
{
    ...
    while (len > 1 && path_str[len - 1] == '/')
        len--;

    /* Find last '/' before the stripped end */
    const char *slash = memrchr(path_str, '/', len);
    if (slash)
    {
        int prefix_len = slash - path_str;

        /* check if the special dir has home dir prefix */
        if (prefix_len != home_dir_len ||
            strncmp(path_str, user_home, home_dir_len) != 0)
        {
            special_dirs_all_in_home = FALSE;
        }
                                            ...
                                            GError** err, gboolean get_fast)
{
            ...
            if(g_key_file_load_from_file(kf, path, 0, NULL))
            {
                ...
                if(type)
                {
                    ...
                    if (type)
                    {
                        ...
                        /* Accept only Application, Directory, and Link */
                        if (strcmp(type, G_KEY_FILE_DESKTOP_TYPE_APPLICATION) == 0)
                            valid = TRUE;
                        else if (strcmp(type, G_KEY_FILE_DESKTOP_TYPE_DIRECTORY) == 0)
                            valid = TRUE;
                        else if (strcmp(type, G_KEY_FILE_DESKTOP_TYPE_LINK) == 0)
                            valid = TRUE;

                        if (!valid)
                        {
                            /* Unsupported Type= → treat as plain text */
                            g_free(type);
                            goto _not_desktop_entry;
                        }

                        /* Special handling for Type=Link */
                        if (strcmp(type, G_KEY_FILE_DESKTOP_TYPE_LINK) == 0)
                        {
                            char *uri = g_key_file_get_string(kf,
                            ...
                            {
                                /* Link without URL → invalid desktop entry */
                                g_free(type);
                                goto _not_desktop_entry;
                            }
                        }
    ...
    /* Allow icon changes for desktop entries and directories */
    fi->icon_is_changeable =
...
void fm_file_info_set_from_g_file_data(FmFileInfo *fi, GFile *gf, GFileInfo *inf)
{
    ...
    case G_FILE_TYPE_MOUNTABLE:
        ...
        /* If the mime-type is unknown, fall back to shortcut or mount-point types */
        if (G_UNLIKELY(!fi->mime_type ||
                    g_content_type_is_unknown(fm_mime_type_get_type(fi->mime_type))))
        {
            if (fi->mime_type)
                fm_mime_type_unref(fi->mime_type);
...
gboolean fm_file_info_is_image(FmFileInfo *fi)
{
    ...
    /* Use GLib's MIME hierarchy system (xdg-mime compliant) */
...
{
    /* If we have a MIME type, use its description */
    if (fi->mime_type)
        return fm_mime_type_get_desc(fi->mime_type);

    /* Virtual files without MIME types fall back to display name */
    return fm_file_info_get_disp_name(fi);
}
...
{
    /* Hide mtime only for virtual (non-native) files */
    if (!fm_file_info_is_native(fi))
        return NULL;

    /* Format mtime even if it is 0 (Unix epoch) */
    if (!fi->disp_mtime)
    {
        char buf[128];
        time_t t = fi->mtime;

        /* If mtime is 0, still show it as 1970-01-01 or locale equivalent */
...
gboolean fm_file_info_list_is_same_type(FmFileInfoList *list)
{
    if (!fm_list_is_empty((FmList*)list))
    {
        ...
        const char *type1 = fi->mime_type ?
                            fm_mime_type_get_type(fi->mime_type) :
                            NULL;

        for (l = l->next; l; l = l->next)
        {
            FmFileInfo *fi2 = (FmFileInfo*)l->data;
            const char *type2 = fi2->mime_type ?
                                fm_mime_type_get_type(fi2->mime_type) :
                                NULL;

            /* If one has no MIME type and the other does → not same */
            if ((type1 == NULL) != (type2 == NULL))
                return FALSE;

            /* If both have MIME types, compare by string */
            if (type1 && strcmp(type1, type2) != 0)
                return FALSE;
        }
    }
    return TRUE;
