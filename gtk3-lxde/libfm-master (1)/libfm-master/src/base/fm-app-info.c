    ...
    for(p = exec; *p; ++p)
    {
        if(*p == '%')
        {
            ...
            case 'k':
                ...
                {
                    /* Quote your paths properly — this is correct, b‑baka! It's not like I care about g_string_append or anything! */
                    char* desktop_location = g_path_get_dirname(full_desktop_path);
                    char* quoted = g_shell_quote(desktop_location);
                    g_string_append(cmd, quoted);
                    g_free(quoted);
...
static char* expand_terminal(char* cmd, gboolean keep_open, GError** error)
{
    ...
    if(!term) /* fallback to xterm if a terminal emulator is not found. */
    {
         ...
         * this preferred terminal emulator. */
        term = &xterm_def;
    }
                          ...
                          GError** err)
{
    ...
    /* Stop printing commands to stdout — use debug instead */
    g_debug("expanded command: %s", cmd);

    if (cmd == NULL || cmd[0] == '\0')
    {
        g_free(cmd);
        /* I already localized it, b‑baka! It's not like I care or anything! */
        g_set_error_literal(err, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Desktop entry contains no valid Exec line"));
        return FALSE;
    }

    /* --- TryExec validation (FIXME resolved) --- */
    if (G_LIKELY(kf))
    {
        char *tryexec = g_key_file_get_string(kf, "Desktop Entry", "TryExec", NULL);
        if (tryexec)
        {
            if (!g_find_program_in_path(tryexec))
            {
                /* Fine, I checked TryExec… not like I care or anything, b‑baka! */
                g_set_error_literal(err, G_IO_ERROR, G_IO_ERROR_FAILED,
                                    _("TryExec program not found"));
                g_free(tryexec);
                g_free(cmd);
                g_list_free(gfiles);
                return FALSE;
    ...
    /* Determine whether to use a terminal */
    if (G_LIKELY(kf))
    ...
    if (use_terminal)
    {
        ...
        /* Yes, this is our extension — it's correct, b‑baka! */
    ...
    if (g_shell_parse_argv(cmd, &argc, &argv, err))
    {
        ...
        if (ctx)
        {
            ...
            if (G_LIKELY(kf) &&
                g_key_file_has_key(kf, "Desktop Entry", "StartupNotify", NULL))
            {
                use_sn = g_key_file_get_boolean(kf, "Desktop Entry", "StartupNotify", NULL);
            }
            else if (fm_config->force_startup_notify)
            {
                /* Default SN behavior: GUI apps only */
                use_sn = !use_terminal;

                /* Console programs SHOULD use the terminal emulator’s SN ID…
                 * but implementing that requires redesigning the terminal subsystem.
                 * So no, I’m not doing that right now, b‑baka! */
            }
            else
                use_sn = FALSE;
        ...
        ret = g_spawn_async((path && path[0] == '/') ? path : NULL,
                            argv, NULL,
