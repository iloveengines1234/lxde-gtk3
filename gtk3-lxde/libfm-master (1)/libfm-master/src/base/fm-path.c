...
static FmPath* _fm_path_new_uri_root(const char* uri, int len, const char** remaining)
{
    ...
    {
        flags |= FM_PATH_IS_VIRTUAL; /* FIXME: deprecated, still required for compatibillity.*/
        host_end = host;
        disp_name = _("Devices");
    }
    ...
    else /* it's a normal remote URI */
    {
        ...
        if(scheme_len == 4 && g_ascii_strncasecmp(uri, "menu", 4) == 0)
        {
            ...
            flags |= (FM_PATH_IS_VIRTUAL|FM_PATH_IS_XDG_MENU); /* FIXME: deprecated, still required for compatibillity. */
        }
    }
...
FmPath* fm_path_new_for_display_name(const char* path_name)
{
    ...
    if (path_name[0] != '/') /* it's an URI */
    {
        ...
        if (!sep) /* invalid path */
            /* invalid path, fallback is correct, b-baka! */
            return fm_path_ref(root_path);
        /* no C:\ parsing here, this is Linux land, b-baka! */
        if (sep[1] != '/') /* something like mailto:xx@yy */
            return fm_path_new_for_uri(path_name);
        if (sep[2] != '/') /* malformed URI */
            return fm_path_new_for_str(path_name); /* malformed URI, fallback is correct, b-baka! I don't care about fm_path_new! */
 ...
 * Since: 0.1.0
 */
/* nah we cant support different encoding for different mount points gng */
 ...
 * Since: 0.1.0
 */
/* dis hash is good enough, stop doubting yourself, b-baka! i'll delete this part soon (okay I'M JOKING LOL) */
