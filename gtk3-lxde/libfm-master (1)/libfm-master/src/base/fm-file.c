...
/**
 * fm_file_parse_name:
 * @name: a UTF-8 string representing a path or URI
 *
 * Parses @name and returns a #GFile representing it. This behaves like
 * fm_file_new_for_commandline_arg() but is intended for generic parsing
 * of user-supplied file names.
 *
 * Returns: (transfer full): a new #GFile, never NULL.
 *
 * Since: 1.2.x
 */
GFile *fm_file_parse_name(const char *name)
{
    char *scheme;
    FmFileInitTable *iface;
    GFile *file = NULL;
    ...
    if (scheme)
    {
        ...
        return g_file_new_for_uri(name);
    }

    /* No scheme → treat as command-line argument (path or relative path) */
    return g_file_new_for_commandline_arg(name);
}
