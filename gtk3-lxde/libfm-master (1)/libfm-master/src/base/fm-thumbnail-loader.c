...
static gboolean is_thumbnail_outdated(GObject* thumb_pix, const char* thumbnail_path, time_t mtime)
{
    ...
    /* No need to compare thumbnail URI with value from tEXt::Thumb::URI */
    ...
    else
    {
        ...
        /* FIXME: XDG specification requires to regenerate it in such case, but nope let's not implement this. */
        struct stat statbuf;
    ...
    *(large_basename - 1) = '/';
    ...
    for(;;)
    {
        ...
        if(G_LIKELY(task))
        {
            ...
            /* we don't need to support fail/<PRG>/<MD5>.png to skip creation */
...
static GObject* scale_pix(GObject* ori_pix, int size)
{
    ...
    else
    {
         ...
         * Don't just fail creating the thumbnail for the image. */
        if(new_width == 0)
            new_width = 1;
        if(new_height == 0)
            new_height = 1;
...
static gboolean generate_thumbnails_with_builtin(ThumbnailTask* task)
{
    /* Built-in decoding already fails gracefully for unsupported formats. */
                                ...
                                const char* output_file, guint size)
{
    ...
    if(_pid <= 0) /* failed to launch */
        /* Don't print error message from failed thumbnailer, let it fallback to a generic icon.*/
        return FALSE;
    g_mutex_lock(lock_ptr);
