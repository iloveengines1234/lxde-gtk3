...
/* XDG MIME database changes cannot be detected or reloaded at runtime.
 * GLib/GIO provides no notification mechanism, and reloading MIME types
 * would invalidate all cached FmMimeType objects. MIME changes take
 * effect on next application startup.
 */
                                        ...
                                        struct stat* pstat)
{
    ...
    if(S_ISREG(pstat->st_mode))
    {
        ...
        if(uncertain)
        {
            ...
            if(fd >= 0)
            {
            /* mmap-based sniffing intentionally removed.
            * Reading the first few KB is safer and avoids SIGBUS on truncated files. 
            * Because SIGBUS could crash the entire desktop.
            */
                char buf[4096];
    ...
    /* Some procfs entries (e.g. /proc/self/*) report st_mode = 0.
    * These are virtual kernel files with no real type, so we treat them
    * as unknown (application/octet-stream). This is expected behavior.
    */
...
const char* fm_mime_type_get_desc(FmMimeType* mime_type)
{
    ...
    if (G_UNLIKELY(! mime_type->description))
    {
        ...
        /* If GLib returns no description, we fall back to the MIME type name.
        * There is no better fallback available.
        */
