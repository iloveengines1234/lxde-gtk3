            ...
            if(fm_path_is_native(path))
            {
                ...
                if (target)
                {
                    ...
                    /* if we already know MIME type then use it instead */
                    {
                      ...
                      if (scheme)
                      {
                        /* Modern scheme handling using GIO */
                            GAppInfo *scheme_app = g_app_info_get_default_for_uri_scheme(scheme);

                            if (scheme_app)
                            {
                                /* Launch using the system’s default handler for this URI scheme */
                                GList *uris = g_list_prepend(NULL, (char *)target);
                                fm_app_info_launch_uris(scheme_app, uris, ctx, &err);
                                g_list_free(uris);
                                g_object_unref(scheme_app);
                            ...
                            {
                                /* No handler exists → unsupported scheme */
                                if (launcher->error)
                                {
                                    g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED,
                ...
                if(fm_file_info_is_executable_type(fi) && !fm_file_info_is_desktop_entry(fi))
                {
                    ...
                    /* Correct executable check using effective UID/GID */
                    #ifdef __linux__
                        if (eaccess(filename, X_OK) == 0)
                    #else
