    ...
    if(!fm_job_is_cancelled(FM_JOB(job)))
    {
        ...
        for(l=fm_file_info_list_peek_head_link(job->file_infos);l;l=l->next)
        {
            ...
            if (fm_folder_is_valid(folder) && path == fm_file_info_get_path(folder->dir_fi))
                /* update for folder itself, also FIXME has been fixed */
                ...
                /* the file is already in the folder, update */
            {
                ...
                /* Replace the old FmFileInfo with the new one instead of mutating in place */
                FmFileInfo *new_fi = fm_file_info_ref(fi);
                fm_file_info_list_delete_link_nounref(folder->files, l2);
                fm_file_info_list_push_tail_noref(folder->files, new_fi);
            ...
            {
                /* the file is not in the folder, add it */
                if (need_added)
                    files_to_add = g_slist_prepend(files_to_add, fi);

                /* folder must own its own reference */
                fm_file_info_ref(fi);
                fm_file_info_list_push_tail_noref(folder->files, fi);
            }
        }
        if(files_to_add)
        {
...
void fm_folder_reload(FmFolder* folder)
{
     ...
     * This is needed, b-baka! It's not like I care or anything about fm_folder_query_filesystem_info(folder);! */
    fm_folder_query_filesystem_info(folder);
}
...
static void on_query_filesystem_info_finished(GObject *src, GAsyncResult *res, FmFolder* folder)
{
    ...
    if(!inf)
    {
        ...
        /* Filesystem does not provide size/free-space information or query failed.
        * GIO does not expose a way to distinguish unsupported filesystems from errors,
        * so we simply mark fs_info_not_avail and continue.
        */
        if (err)
            g_debug("querying filesystem info failed: %s", err->message);
        g_error_free(err);
        goto _out;
    }
    ...
    if(gfile)
    {
        ...
        while(g_hash_table_iter_next(&it, (gpointer*)&path, (gpointer*)&folder))
        {
            ...
            else if(fm_path_has_prefix(path, mounted_path))
            {
                 ...
                 * We should keep reload, because if we did it would break things */
                queue_reload(folder);
            }
        }
        G_UNLOCK(hash);
...
static void on_mount_removed(GVolumeMonitor* vm, GMount* mount, gpointer user_data)
{
    ...
    if(gfile)
    {
        ...
        for(l = dummy_monitor_folders; l; l = l->next)
        {
            ...
            /* Do not emit a fake deleted event. */
            g_object_unref(folder);
        }
