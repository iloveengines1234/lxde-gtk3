...
#undef HIDE_WIDGET
    ...
    /* row 0: "Exec" GtkBox: GtkEntry+GtkButton */
    new_widget = gtk_label_new(NULL);
    label = GTK_LABEL(new_widget);
    
    /* Replaced gtk_misc_set_alignment with modern GtkWidget alignment flags */
    gtk_widget_set_halign(new_widget, GTK_ALIGN_START);
    gtk_widget_set_valign(new_widget, GTK_ALIGN_START);
    
    gtk_label_set_markup_with_mnemonic(label, _("<b>Co_mmand:</b>"));
    
    /* Replaced gtk_table_attach with gtk_grid_attach */
    /* Parameters: (grid, widget, column, row, width, height) */
    gtk_grid_attach(table, new_widget, 0, 0, 1, 1);

    /* Cleaned up legacy version checks, using pure GTK3 hardware-accelerated GtkBox */
