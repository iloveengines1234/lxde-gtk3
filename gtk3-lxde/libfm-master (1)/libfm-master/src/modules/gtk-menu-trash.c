                                        ...
                                        gboolean single_file)
{
    ...
    for(l = fm_file_info_list_peek_head_link(files); l; l = l->next)
    {
        ...
        FmPath *parent_path = fm_path_get_parent(trash_path);
        if(!parent_path || !fm_path_is_trash_root(parent_path))
        {
            can_restore = FALSE;
            break;
        }
...
FmContextMenuSchemeAddonInit fm_module_init_gtk_menu_scheme = {
    .init = NULL,
    .finalize = NULL,
    .update_file_menu_for_scheme = _update_file_menu_for_trash,
    .update_folder_menu = _update_folder_menu_for_trash
};