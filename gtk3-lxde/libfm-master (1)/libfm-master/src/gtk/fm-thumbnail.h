 ...
 *
 * This callback is called when the thumbnail request is completed, 
 * either successfully or with an error.
 * The callback is invoked from a worker thread, not the GTK main loop.
 * If the callback needs to interact with GTK widgets, it must schedule
 * that work back to the main thread using g_idle_add() or
