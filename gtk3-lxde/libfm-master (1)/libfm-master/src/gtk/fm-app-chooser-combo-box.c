...
{
    FmAppChooserComboBoxData* data = user_data;

    GtkTreePath *a = gtk_tree_model_get_path(model, &data->separator_iter);
    GtkTreePath *b = gtk_tree_model_get_path(model, it);

    gboolean equal = (gtk_tree_path_compare(a, b) == 0);
...
void fm_app_chooser_combo_box_setup(GtkComboBox* combo, FmMimeType* mime_type, GList* apps, GAppInfo* sel)
{
    ...
    gtk_list_store_insert_with_values(store, &it, -1,
                       0, NULL,
                       1, _("Customize…"), // You only had to type 3 dots gng, you b-baka!
                       2, NULL, -1);
