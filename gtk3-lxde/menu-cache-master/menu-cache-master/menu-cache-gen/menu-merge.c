...
/* External or global control variables assumed from context */
extern gint verbose; 
...
static void _add_app_dir(const char *app_dir)
{
    ...
    if (dir)
    {
        ...
        {
            char *path = g_build_filename(app_dir, str, NULL);
                                         ...
                                         GError **error, gpointer user_data)
{
    ...
    else
    {
        ...
        _path = g_build_filename(_dir, path, NULL);
        path = _path;
        g_free(_dir);
...
/* External Macro assumed for debug monitoring output */
extern void VDBG(const char *format, ...);
...
replace_from_system_config_dirs:
                        fm_xml_file_item_destroy(name);
                        while (*rel_path == G_DIR_SEPARATOR) 
                            rel_path++;
                        while (dirs[0] != NULL)
                        {
                            if (dirs[0] == dir[0])
...
static MenuLayout *_find_layout(FmXmlFileItem *item, gboolean create)
{
    ...
    {
        /* Porting Note: Replaced deprecated g_slice with standard g_new0 */
        layout = g_new0(MenuLayout, 1);
        /* set defaults */
...
restart:
    ...
    if (sub != NULL)
    {
        ...
        /* Porting Note: Fixed const correctness casting for modern GLib toolchains */
        int i = g_strv_length((gchar **)dirs);

        /* insert in reverse order - see XDG menu specification */
        while (i > 0)
        {
            merged_dir = g_build_filename(dirs[--i], "menus", "applications-merged", NULL);
            it_sub = fm_xml_file_item_new(menuTag_MergeDir);
            fm_xml_file_item_append_text(it_sub, merged_dir, -1, FALSE);
        ...
        merged_dir = g_build_filename(g_get_user_config_dir(), "menus", "applications-merged", NULL);
        it_sub = fm_xml_file_item_new(menuTag_MergeDir);
        fm_xml_file_item_append_text(it_sub, merged_dir, -1, FALSE);
        if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
        {
    ...
    if (sub != NULL)
    {
        ...
        while (i > 0)
        {
            merged_app = g_build_filename(dirs[--i], "applications", NULL);
            it_sub = fm_xml_file_item_new(menuTag_AppDir);
            fm_xml_file_item_append_text(it_sub, merged_app, -1, FALSE);
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
        ...
        merged_app = g_build_filename(g_get_user_data_dir(), "applications", NULL);
        it_sub = fm_xml_file_item_new(menuTag_AppDir);
        fm_xml_file_item_append_text(it_sub, merged_app, -1, FALSE);
        if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
        {
    ...
    if (sub != NULL)
    {
        ...
        while (i > 0)
        {
            merged_kde = g_build_filename(dirs[--i], "applnk", NULL);
            it_sub = fm_xml_file_item_new(menuTag_LegacyDir);
            fm_xml_file_item_set_comment(it_sub, "kde-");
            fm_xml_file_item_append_text(it_sub, merged_kde, -1, FALSE);
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
        ...
        merged_kde = g_build_filename(g_get_user_data_dir(), "applnk", NULL);
        it_sub = fm_xml_file_item_new(menuTag_LegacyDir);
        fm_xml_file_item_set_comment(it_sub, "kde-");
        fm_xml_file_item_append_text(it_sub, merged_kde, -1, FALSE);
        if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
    ...
    if (sub != NULL)
    {
        ...
        while (i > 0)
        {
            merged_dir_dir = g_build_filename(dirs[--i], "desktop-directories", NULL);
            it_sub = fm_xml_file_item_new(menuTag_DirectoryDir);
            fm_xml_file_item_append_text(it_sub, merged_dir_dir, -1, FALSE);
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                            _("Failed to insert tag <DirectoryDir>%s</DirectoryDir>"),
                            merged_dir_dir);
                g_free(merged_dir_dir);
                goto failed; /* failed to merge */
            }
            g_free(merged_dir_dir);
        }
        merged_dir_dir = g_build_filename(g_get_user_data_dir(), "desktop-directories", NULL);
        it_sub = fm_xml_file_item_new(menuTag_DirectoryDir);
        fm_xml_file_item_append_text(it_sub, merged_dir_dir, -1, FALSE);
        if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0)
        {
...
/* Global tracking table layout hook assumed from contextual definitions */
static GHashTable *layout_hash = NULL;
...
static GList *_layout_items_copy(GList *orig)
{
    ...
    while (orig)
    {
        ...
        case MENU_CACHE_TYPE_DIR:
            ...
            MC_VVDBG("*** new menu layout: MenuMenuname %s", ((MenuMenuname *)item)->name);
            break;
        default:
            item = NULL;
            break;
...
static MenuMenu *_make_menu_node(FmXmlFileItem *node, MenuLayout *def)
{
    ...
    for (l = children; l; l = l->next)
    {
        ...
        if (tag == menuTag_Menu)
        {
            MenuMenu *child = _make_menu_node((FmXmlFileItem *)l->data, def);
            if (child != NULL)
            {
                MC_VDBG("*** added submenu %s", child->name);
                menu->children = g_list_prepend(menu->children, child);
            }
        }
        else if (tag == menuTag_Directory)
        {
            item = fm_xml_file_item_find_child((FmXmlFileItem *)l->data, FM_XML_FILE_TEXT);
            if (item != NULL)
        ...
        {
            if (menu->name == NULL)
                menu->name = g_strdup(fm_xml_file_item_get_data(fm_xml_file_item_find_child((FmXmlFileItem *)l->data,
                                                                                            FM_XML_FILE_TEXT), NULL));
        }
                 ...
                 /* FIXME: can those be here? Filename Category All And Not Or */
        {
            ...
            child->rule = (FmXmlFileItem *)l->data;
            MC_VDBG("*** adding rule %s", fm_xml_file_item_get_tag_name((FmXmlFileItem *)l->data));
            menu->children = g_list_prepend(menu->children, child);
        }
    }
    g_list_free(children);
    menu->children = g_list_reverse(menu->children);
    MC_VDBG("*** done menu %s", menu->name);
    return menu;
}
...
static FmXmlFileItem *_find_in_children(GList *list, const char *name)
{
    while (list)
    {
        const char *elem_name = _get_menu_name((FmXmlFileItem *)list->data);
        if (g_strcmp0(elem_name, name) == 0)
            return (FmXmlFileItem *)list->data;
        else
            list = list->next;
    }
    return NULL;
...
{
    MenuLayout *layout = (MenuLayout *)data;
...
MenuMenu *get_merged_menu(const char *file, FmXmlFile **xmlfile, GError **error)
{
    ...
    /* Load the file via modern GFile APIs safely */
    data.file_path = file;
    ...
    layout_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, _free_layout);
