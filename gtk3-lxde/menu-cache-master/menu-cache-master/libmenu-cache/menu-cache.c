...
/* Clean, modern GLib 2.32+ thread sync wrappers */
...
{
    /* g_type_init() is completely obsolete and removed in modern GLib frameworks */
}
...
_fail:
        ...
        if(item->type == MENU_CACHE_TYPE_DIR)
            g_slice_free(MenuCacheDir, (MenuCacheDir*)item);
        else
            g_slice_free(MenuCacheApp, (MenuCacheApp*)item);
        return NULL;
    }
    idx = atoi( line );
    g_free(line);
    ...
    if( item->type == MENU_CACHE_TYPE_DIR )
        read_dir( f, (MenuCacheDir*)item, cache, all_used_files, n_all_used_files );
    else if( item->type == MENU_CACHE_TYPE_APP )
        read_app( f, (MenuCacheApp*)item, cache );
 ...
 * @cache: a menu cache descriptor
 *
 * Decreases reference counter on @cache. When reference count becomes 0
 ...
 */
static gboolean delayed_item_unref(gpointer data)
{
    if (data)
        menu_cache_item_unref(MENU_CACHE_ITEM(data));
...
MenuCacheDir* menu_cache_get_root_dir( MenuCache* cache )
{
    ...
    if(dir)
        g_timeout_add_seconds(10, delayed_item_unref, dir);
    return dir;
}
...
gboolean menu_cache_item_unref(MenuCacheItem* item)
{
    ...
    if( g_atomic_int_dec_and_test( &item->n_ref ) )
    {
        ...
        else
        {
            ...
            g_strfreev((gchar **)app->categories);
            g_free(app->keywords);
...
{
    return item ? item->type : MENU_CACHE_TYPE_SEP;
}
 ...
 * @item: a menu cache item
 *
 * Retrieves path to directory where @item desktop entry file is located.
 *
...
{
    return (item && item->file_dir) ? item->file_dir->dir + 1 : NULL;
}
 ...
 * @item: a menu cache item
 *
 * Retrieves path to @item desktop entry file. This API can return %NULL
 * if @item is a directory and has no desktop entry file. Returned data
...
{
    if( ! item || ! item->file_name || ! item->file_dir )
        return NULL;
    return g_build_filename( item->file_dir->dir + 1, item->file_name, NULL );
}

/* Helper function matching GSourceFunc signature exactly to prevent casting warnings */
static gboolean delayed_item_unref(gpointer data)
{
    if (data)
        menu_cache_item_unref(MENU_CACHE_ITEM(data));
...
GSList* menu_cache_dir_get_children( MenuCacheDir* dir )
{
    ...
    g_timeout_add_seconds(10, delayed_item_unref,
                          menu_cache_item_ref(MENU_CACHE_ITEM(dir)));
    return dir->children;
}
...
GSList* menu_cache_dir_list_children(MenuCacheDir* dir)
{
    ...
    if(!dir || MENU_CACHE_ITEM(dir)->type != MENU_CACHE_TYPE_DIR)
        return NULL;
    MENU_CACHE_LOCK;
    children = g_slist_copy(dir->children);
    for(l = children; l; l = l->next)
        menu_cache_item_ref((MenuCacheItem*)l->data);
    MENU_CACHE_UNLOCK;
    return children;
}
...
MenuCacheItem *menu_cache_find_child_by_id(MenuCacheDir *dir, const char *id)
{
    ...
    if (!dir || MENU_CACHE_ITEM(dir)->type != MENU_CACHE_TYPE_DIR || id == NULL)
        return NULL;
    MENU_CACHE_LOCK;
    for (child = dir->children; child; child = child->next)
    {
        ...
        {
            item = menu_cache_item_ref((MenuCacheItem*)child->data);
            break;
        }
    }
    MENU_CACHE_UNLOCK;
...
MenuCacheItem *menu_cache_find_child_by_name(MenuCacheDir *dir, const char *name)
{
    ...
    if (!dir || MENU_CACHE_ITEM(dir)->type != MENU_CACHE_TYPE_DIR || name == NULL)
        return NULL;
    MENU_CACHE_LOCK;
    for (child = dir->children; child; child = child->next)
    {
...
{
    return (dir && (dir->flags & FLAG_IS_NODISPLAY) == 0);
}
...
{
    return app ? app->generic_name : NULL;
}
...
{
    return app ? app->working_dir : NULL;
}
...
{
    return app ? (const char * const *)app->categories : NULL;
}
...
{
    return (app && (app->flags & FLAG_USE_TERMINAL) != 0);
}
...
{
    return (app && (app->flags & FLAG_USE_SN) != 0);
}
...
{
    return app ? app->show_in_flags : 0;
}
...
{
    if(!app || (app->flags & FLAG_IS_NODISPLAY))
        return FALSE;
...
MenuCacheDir* menu_cache_get_dir_from_path( MenuCache* cache, const char* path )
{
    ...
    if( !cache || !path || path[0] != '/' )
        return NULL;

    names = g_strsplit( path + 1, "/", -1 );
    if( !names )
        return NULL;
...
MenuCacheItem* menu_cache_item_from_path( MenuCache* cache, const char* path )
{
    ...
    for( i = 1; names[i]; ++i )
    {
        ...
        for( ; l; l = l->next )
        {
            MenuCacheItem* child_item = MENU_CACHE_ITEM(l->data);
            if( g_strcmp0( child_item->id, names[i] ) == 0 )
            {
                item = child_item;
                if( item->type == MENU_CACHE_TYPE_DIR )
...
/* Forward declaration needed since the runtime context expects it */
static gboolean connect_server(GCancellable* cancellable);
...
static void unregister_menu_from_server( MenuCache* cache )
{
    ...
    {
        /* Log or handle socket write error if necessary */
    }
}
 ...
 * such connection exist then creates new one.
 *
...
guint32 menu_cache_get_desktop_env_flag( MenuCache* cache, const char* desktop_env )
{
    ...
    if (!cache || desktop_env == NULL || desktop_env[0] == '\0')
        return flags;
    ...
    if (item)
    {
        switch (menu_cache_item_get_type(item))
        {
            case MENU_CACHE_TYPE_DIR:
                ...
                {
                    MenuCacheItem *found = _scan_by_id(MENU_CACHE_ITEM(l->data), id);
                    if (found)
                        return found;
                }
                break;
...
static GSList* list_app_in_dir_for_kw(MenuCacheDir *dir, GSList *list, const char *kw)
{
    ...
    for (l = dir->children; l; l = l->next)
    {
        ...
        case MENU_CACHE_TYPE_DIR:
            ...
            break;
        case MENU_CACHE_TYPE_APP:
            if (MENU_CACHE_APP(item)->keywords && strstr(MENU_CACHE_APP(item)->keywords, kw) != NULL)
                list = g_slist_prepend(list, menu_cache_item_ref(item));
            break;
        case MENU_CACHE_TYPE_NONE:
        case MENU_CACHE_TYPE_SEP:
...
GSList *menu_cache_list_all_for_keyword(MenuCache* cache, const char *keyword)
{
    ...
    casefolded = g_utf8_casefold(keyword, -1);
    if (!casefolded)
        return NULL;
