...
void kf_get_int(GKeyFile* kf, const char* grp, const char* key, int* val)
{
    gint ret;
    GError* err = NULL;
    ret = g_key_file_get_integer(kf, grp, key, &err);
    if( !err )
        *val = ret;
    else
        g_error_free(err);
...
gchar* utf8_strcasestr( const char* s1, const char* s2 )
{
    ...
    if (found)
    {
        /* Calculate string offset to avoid returning a pointer to freed memory */
        glong offset = g_utf8_pointer_to_offset(p1, found);
        result = (gchar *)s1 + g_utf8_offset_to_pointer(s1, offset) - s1;
    }
...
gchar* guess_title_from_url (const char *url) 
{
    ...
    if (decoded_val)
    {
        slash = g_utf8_strrchr ( decoded_val, -1, '/' );
        if (slash)
            title = g_strdup( slash + 1 );
        else
            title = g_strdup( decoded_val );
...
/* helper function: most likely we want to unref directly after
