...
const gchar* plugin_config_gettext(const gchar *lookup) 
{
    ...
    /* The strings in the hash table are already wrapped with _() when inserted */
    const gchar *message = g_hash_table_lookup( translation, lookup );
    return message;
}
...
void plugin_config_setup (xmmsc_connection_t *con) 
{
    ...
    /* we haven't entered async xmmsc_mainloop, so it's ok to do sync ops */
