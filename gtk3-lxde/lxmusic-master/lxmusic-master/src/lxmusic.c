...
/* Modern System Tray Handling via AppIndicator instead of broken GtkStatusIcon */
...
static AppIndicator *tray_icon = NULL; /* Ported from GtkStatusIcon */
...
/* Ported from GtkTable to modern GtkGrid */
void on_pref_output_plugin_changed(GtkGrid* grid, GtkComboBox* output)
{
    ...
    /* destroy previous configuration widgets safely */
    ...
    for ( row = 0; row < n_configs; row++ ) 
    {
        ...
        /* Fixed: Modern widget alignments replace GtkMisc alignment flags */
        gtk_widget_set_halign(label, GTK_ALIGN_START);
        gtk_widget_set_valign(label, GTK_ALIGN_CENTER);
        
        gtk_widget_set_hexpand(config->entry, TRUE);

        /* Ported: Using clean Grid coordinate attachment logic */
        gtk_grid_attach(grid, label, 0, row, 1, 1);
        gtk_grid_attach(grid, config->entry, 1, row, 1, 1);
...
int on_pref_dlg_init_output_plugin(xmmsv_t* value, void* user_data)
{
    ...
    for ( i = 0; i < g_list_length( plugin_list() ); i++ ) 
    {
        ...
        GtkListStore *current_store = GTK_LIST_STORE(gtk_combo_box_get_model(output_plugin_cb));
        GtkTreeIter iter;
        gtk_list_store_append(current_store, &iter);
        gtk_list_store_set(current_store, &iter, 0, label, -1);
    }
...
static void on_tray_icon_activate(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    {
        gtk_window_move((GWindow*)main_win, win_xpos, win_ypos);
...
/* Reconstructed tray system setup loop built exclusively via AppIndicator */
static void create_tray_menu_and_icon()
{
    ...
    /* Ported from Stock Items to standard Modern Named Icons */
    item = gtk_menu_item_new_with_label(_("Show LXMusic"));
    GtkWidget *show_img = gtk_image_new_from_icon_name("document-open", GTK_ICON_SIZE_MENU);
    // Note: If you want icons inside menus in GTK3, custom boxes can be packed, 
    // but a standard clean textual menu item layout matches standard panel specs.
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_show_main_win), NULL);
    ...
    if (playback_status == XMMS_PLAYBACK_STATUS_PLAY) {
        ...
        item = gtk_menu_item_new_with_label(_("Play"));
    }
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_play_btn_clicked), NULL);

    item = gtk_menu_item_new_with_label(_("Stop"));
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_stop_btn_clicked), NULL);

    item = gtk_menu_item_new_with_label(_("Previous"));
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_prev_btn_clicked), NULL);

    item = gtk_menu_item_new_with_label(_("Next"));
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_next_btn_clicked), NULL);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(systray_menu), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_quit), NULL);

    gtk_widget_show_all(systray_menu);

    /* Initialize AppIndicator instead of legacy StatusIcon */
    tray_icon = app_indicator_new("lxmusic-indicator",
                                   "lxmusic",
                                   APP_INDICATOR_CATEGORY_APPLICATION_STATUS);
    
    app_indicator_set_status(tray_icon, APP_INDICATOR_STATUS_ACTIVE);
    app_indicator_set_menu(tray_icon, GTK_MENU(systray_menu));
    app_indicator_set_title(tray_icon, _("LXMusic"));
}

static void create_tray_icon()
{
    create_tray_menu_and_icon();
}
...
void on_preference(GtkAction* act, gpointer data)
{
    ...
    /* Fixed: Points cleanly to your upgraded .ui file track extension */
...
static int on_coll_info_received(xmmsv_t* value, void* user_data) 
{
    ...
    for ( i = 0; i < xmmsv_list_get_size( value ); i++ )
    {
        ...
        {
            xmmsv_get_int ( id_val, &track_id );
            g_hash_table_insert( id_to_coll_info, GINT_TO_POINTER( track_id ), track_info );
        }
    }
...
void on_file_properties(GSimpleAction* act, GVariant* parameter, gpointer data)
{
...
{
    enum { RESPONSE_ADD = 1 };
    GtkWidget *dlg = gtk_file_chooser_dialog_new( _("Add Files"), (GtkWindow*)main_win,
                                                  GTK_FILE_CHOOSER_ACTION_OPEN,
                                                  _("_Cancel"), GTK_RESPONSE_CANCEL,
                                                  _("_Add"), RESPONSE_ADD, NULL );
    GtkFileFilter* filter;

    gtk_window_set_destroy_with_parent(GTK_WINDOW(dlg), TRUE);
    gtk_file_chooser_set_select_multiple( (GtkFileChooser*)dlg, TRUE );

    /* add a custom filter which filters audio files */
...
{
    gtk_menu_popup_at_widget(GTK_MENU(add_to_pl_menu), GTK_WIDGET(btn), GDK_GRAVITY_SOUTH_WEST, GDK_GRAVITY_NORTH_WEST, NULL);
}
...
void on_remove_all(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    xmmsc_result_t* res;
...
void on_remove_selected(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    if( cur_playlist )
    {
...
{
    gtk_menu_popup_at_widget(GTK_MENU(rm_from_pl_menu), GTK_WIDGET(btn), GDK_GRAVITY_SOUTH_WEST, GDK_GRAVITY_NORTH_WEST, NULL);
}

gboolean on_playlist_view_key_press_event (GtkWidget *widget, GdkEventKey *event)
{
    switch (event->keyval) 
    {
    case GDK_KEY_Delete:
        on_remove_selected (NULL, NULL, NULL); 
        break;
    case GDK_KEY_Insert:
...
static int update_track( xmmsv_t *value, GtkTreeRowReference *ref )
{
    ...
    if (id > 0)
    {
        /* Note: If tray_icon was a GtkStatusIcon, it is deprecated in GTK3. 
           Passing it here as a fallback, but you might need to port notifications separately */
...
static int on_playlist_loaded(xmmsv_t* value, gpointer user_data)
{
    ...
    if( !xmmsv_is_error(value) && xmmsv_get_string(value, (const char**)&name) )
    {
        ...
        /* invalidate currently played track id. */
        cur_track_id = 0;
...
static int on_playback_status_changed( xmmsv_t *value, void *user_data )
{
    ...
    switch( playback_status )
    {
        case XMMS_PLAYBACK_STATUS_PLAY:
        {
            ...
            img = gtk_button_get_image( GTK_BUTTON(play_btn) );
            if ( GTK_IS_IMAGE(img) )
                gtk_image_set_from_icon_name( (GtkImage*)img, "media-playback-pause", GTK_ICON_SIZE_BUTTON );
        ...
        case XMMS_PLAYBACK_STATUS_PAUSE:
            ...
            img = gtk_button_get_image( GTK_BUTTON(play_btn) );
            if ( GTK_IS_IMAGE(img) )
                gtk_image_set_from_icon_name( (GtkImage*)img, "media-playback-start", GTK_ICON_SIZE_BUTTON );
            break;
    }
...
static int on_playback_playtime_changed( xmmsv_t* value, void* user_data )
{
    int32_t time;
    char buf[32];
    if ( xmmsv_is_error(value) || ! xmmsv_get_int(value, &time))
        return TRUE;
    ...
    if( cur_track_duration > 0 )
    {
        ...
        gtk_range_set_value( (GtkRange*)progress_bar, (((gdouble)100000 * time) / cur_track_duration) );
...
static int on_playback_track_loaded( xmmsv_t* value, void* user_data )
{
    ...
    if( tray_icon ) {
G_GNUC_BEGIN_IGNORE_DECORATIONS
        gtk_status_icon_set_tooltip_text(GTK_STATUS_ICON(tray_icon), window_title->str);
G_GNUC_END_IGNORE_DECORATIONS
    }

    /* GtkStatusIcon is deprecated, passing safe pointer fallback configuration */
    LXMusicNotification lxn = lxmusic_do_notify_prepare ( track_properties.artist, track_properties.title,  
                                                              _("Now Playing:"), tray_icon );
    if (!track_properties.picture_front) 
        send_notification( lxn );
    else 
    {
...
void on_locate_cur_track(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    GtkTreeIter cur_track_iter = get_current_track_iter();
    if( cur_track_iter.stamp )
    {
...
static GtkTreeIter get_current_track_iter() 
{
    ...
    if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL(list_store), &it )) 
    {
        do {
            gint weight;
            gtk_tree_model_get (GTK_TREE_MODEL(list_store), &it, COL_WEIGHT, &weight, -1);
            if (weight == PANGO_WEIGHT_BOLD) {
                found_it = it;
                break;
            }
        } while (gtk_tree_model_iter_next( GTK_TREE_MODEL(list_store), &it ));
    }
    return found_it;
}
...
static gboolean on_volume_btn_scrolled(GtkWidget *widget, GdkEventScroll *event, gpointer user_data)
{
    gshort volume;
...
static void setup_ui()
{
    ...
    /* Menus are extracted via GtkBuilder objects directly instead of using GtkUIManager */
    switch_pl_mi = (GtkWidget*)gtk_builder_get_object(builder, "switch_to_pl");
    switch_pl_menu = gtk_menu_new();
    if (GTK_IS_MENU_ITEM(switch_pl_mi)) {
        gtk_menu_item_set_submenu(GTK_MENU_ITEM(switch_pl_mi), switch_pl_menu);
    }

    GtkWidget *add_to_mi = (GtkWidget*)gtk_builder_get_object(builder, "add_to_pl");
    if (GTK_IS_MENU_ITEM(add_to_mi)) {
        add_to_pl_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(add_to_mi));
    }
    
    GtkWidget *rm_from_mi = (GtkWidget*)gtk_builder_get_object(builder, "remove_from_pl");
    if (GTK_IS_MENU_ITEM(rm_from_mi)) {
        rm_from_pl_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(rm_from_mi));
    }
    ...
    if (GTK_IS_CHECK_MENU_ITEM(show_pl_mi)) {
...
void on_new_playlist(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    GtkWidget *dlg = gtk_dialog_new_with_buttons(
...
void on_del_playlist(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    GSList *prev=NULL, *l;
    char* switch_to = NULL;

    if( !switch_pl_menu_group )
        return;
    if( ! cur_playlist )
        return;
...
void on_show_playlist(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    GVariant *state = g_action_get_state(G_ACTION(act));
    show_playlist = g_variant_get_boolean(state);
...
void on_playlist_cut(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    show_error( GTK_WINDOW(main_win), NULL, "Not yet implemented");
}

void on_playlist_copy(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    show_error( GTK_WINDOW(main_win), NULL, "Not yet implemented");
}

void on_playlist_paste(GSimpleAction* act, GVariant* parameter, gpointer user_data)
{
    show_error( GTK_WINDOW(main_win), NULL, "Not yet implemented");
}
