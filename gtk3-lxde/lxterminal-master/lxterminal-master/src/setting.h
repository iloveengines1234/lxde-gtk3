...
/* User preferences structure configuration context */
typedef struct _setting {
...
#endif
    const char * color_preset;  /* Color preset name pointer mapping */
    ...
    /* Shortcut group settings fields */
...
/* Colors use char pointers for Gdk2 and Gdk3 compatibility and less duplication */
...
extern Setting * get_setting(void);
extern void save_setting(const char * profile);
extern Setting * load_setting(const char * profile);

/* Utilities for structural manipulation through preferences subsystem */
extern void set_setting(Setting * new_setting);
extern void free_setting(Setting ** setting_ptr);
extern Setting * copy_setting(Setting * src_setting);

extern void print_setting(void);

/* Array definition tracking boundary properties explicitly */
extern ColorPreset color_presets[];

#endif /* LXTERMINAL_SETTING_H */