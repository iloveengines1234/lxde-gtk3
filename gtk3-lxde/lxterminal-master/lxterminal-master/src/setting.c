...
void save_setting(const char *profile)
{
    ...
    g_return_if_fail (setting->keyfile != NULL);
    
    /* Push settings to GKeyFile. */
    g_key_file_set_string(setting->keyfile, GENERAL_GROUP, FONT_NAME, setting->font_name ? setting->font_name : "");
    ...
    for (i = 0; i < 16; i++) {
        ...
        g_free(palette_color_key);
    }
    
    g_key_file_set_string(setting->keyfile, GENERAL_GROUP, COLOR_PRESET, setting->color_preset ? setting->color_preset : "Custom");
    ...
    g_key_file_set_string(setting->keyfile, GENERAL_GROUP, TAB_POS, setting->tab_position ? setting->tab_position : "top");
    ...
    g_key_file_set_string(setting->keyfile, GENERAL_GROUP, SEL_CHARS, setting->word_selection_characters ? setting->word_selection_characters : "");
    ...
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, NEW_WINDOW_ACCEL, setting->new_window_accel ? setting->new_window_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, NEW_TAB_ACCEL, setting->new_tab_accel ? setting->new_tab_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, CLOSE_TAB_ACCEL, setting->close_tab_accel ? setting->close_tab_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, CLOSE_WINDOW_ACCEL, setting->close_window_accel ? setting->close_window_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, COPY_ACCEL, setting->copy_accel ? setting->copy_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, PASTE_ACCEL, setting->paste_accel ? setting->paste_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, NAME_TAB_ACCEL, setting->name_tab_accel ? setting->name_tab_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, PREVIOUS_TAB_ACCEL, setting->previous_tab_accel ? setting->previous_tab_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, NEXT_TAB_ACCEL, setting->next_tab_accel ? setting->next_tab_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, MOVE_TAB_LEFT_ACCEL, setting->move_tab_left_accel ? setting->move_tab_left_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, MOVE_TAB_RIGHT_ACCEL, setting->move_tab_right_accel ? setting->move_tab_right_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, ZOOM_IN_ACCEL, setting->zoom_in_accel ? setting->zoom_in_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, ZOOM_OUT_ACCEL, setting->zoom_out_accel ? setting->zoom_out_accel : "");
    g_key_file_set_string(setting->keyfile, SHORTCUT_GROUP, ZOOM_RESET_ACCEL, setting->zoom_reset_accel ? setting->zoom_reset_accel : "");

    /* Convert GKeyFile to text and build path to configuration file safely. */
    ...
    if ((file_data != NULL) && (config_path != NULL))
    {
        ...
        /* Atomic write utility safely protects target assets against fragmentation */
        if (!g_file_set_contents(config_path, file_data, -1, &write_error))
        {
...
/* Deep copy settings. */
Setting * copy_setting(Setting * src_setting)
{
    g_return_val_if_fail (src_setting != NULL, NULL);

    /* Allocate structure. */
    Setting * new_setting = g_slice_new0(Setting);
    memcpy(new_setting, src_setting, sizeof(Setting));

    /* Do not share underlying pointer states to keyfile contexts */
    new_setting->keyfile = NULL;

    new_setting->font_name = g_strdup(src_setting->font_name);
    new_setting->tab_position = g_strdup(src_setting->tab_position);
    new_setting->word_selection_characters = g_strdup(src_setting->word_selection_characters);
    
    /* Shortcut group settings. */
    new_setting->new_window_accel = g_strdup(src_setting->new_window_accel);
    new_setting->new_tab_accel = g_strdup(src_setting->new_tab_accel);
    new_setting->close_tab_accel = g_strdup(src_setting->close_tab_accel);
    new_setting->close_window_accel = g_strdup(src_setting->close_window_accel);
    new_setting->copy_accel = g_strdup(src_setting->copy_accel);
    new_setting->paste_accel = g_strdup(src_setting->paste_accel);
    new_setting->name_tab_accel = g_strdup(src_setting->name_tab_accel);
    new_setting->previous_tab_accel = g_strdup(src_setting->previous_tab_accel);
    new_setting->next_tab_accel = g_strdup(src_setting->next_tab_accel);
    new_setting->move_tab_left_accel = g_strdup(src_setting->move_tab_left_accel);
    new_setting->move_tab_right_accel = g_strdup(src_setting->move_tab_right_accel);
    new_setting->zoom_in_accel = g_strdup(src_setting->zoom_in_accel);
    new_setting->zoom_out_accel = g_strdup(src_setting->zoom_out_accel);
    new_setting->zoom_reset_accel = g_strdup(src_setting->zoom_reset_accel);
    
    return new_setting;
}

/* Deep free settings. */
void free_setting(Setting ** setting_ptr)
{
    Setting *_setting;
    g_return_if_fail (setting_ptr != NULL && *setting_ptr != NULL);

    _setting = *setting_ptr;
    ...
    g_free(_setting->zoom_in_accel);
    g_free(_setting->zoom_out_accel);
    g_free(_setting->zoom_reset_accel);

    if (_setting->keyfile != NULL) {
        g_key_file_free(_setting->keyfile);
    }
