...
static gint preset_custom_id = 0;
...
static void preferences_dialog_font_set_event(GtkFontButton * widget, Setting * setting)
{
    ...
    setting->font_name = g_strdup(gtk_font_chooser_get_font(GTK_FONT_CHOOSER(widget)));
#else
...
/* Convert the user preference on tab position to the internal representation. */
...
static gboolean preferences_dialog_shortcut_key_press_event(GtkWidget * widget, GdkEventKey * ekey, gchar ** s)
{
    ...
    for (sib = children; sib != NULL; sib = sib->next) {
        if (GTK_IS_ENTRY(sib->data) && GTK_WIDGET(sib->data) != widget) {
            if (!g_strcmp0(lbl, gtk_entry_get_text(GTK_ENTRY(sib->data)))) {
...
void terminal_preferences_dialog(GtkAction * action, LXTerminal * terminal)
{
    ...
    gtk_spin_button_set_value(GTIN_SPIN_BUTTON(w), setting->scrollback);
    ...
    gtk_entry_set_text(GTK_ENTRY(w), setting->word_selection_characters ? setting->word_selection_characters : "");
...
#undef PREF_SETUP_SHORTCUT

    gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
    if (terminal != NULL && terminal->window != NULL) {
        gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(terminal->window));
    }
    ...
    {
        set_setting(setting);
        save_setting(terminal ? terminal->profile : NULL);
        if (terminal != NULL) {
