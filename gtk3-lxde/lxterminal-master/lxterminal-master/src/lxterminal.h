...
/* Match patterns sourced from tilda-0.09.6/src/tilda_terminal.c:36 */
...
typedef struct _lxterminal {
    ...
    GdkRGBA background_rgba;        /* Modern RGBA tracking fallback */
    ...
    gchar *profile;                 /* Configuration profile name string (owned) */
} LXTerminal;
...
/* Output of lxterminal_process_arguments. All string fields are safely heap-allocated. */
typedef struct _command_arguments {
    ...
    unsigned int geometry_columns;  /* Value of --geometry width */
    unsigned int geometry_rows;     /* Value of --geometry height */
    int geometry_xoff;
    int geometry_yoff;
    char * title;                   /* Value of -t, -T, --title; heap-allocated via g_strdup */
    char * tabs;                    /* Value of --tabs; heap-allocated via g_strdup */
    char * working_directory;       /* Value of --working-directory; heap-allocated via g_strdup */
    char * profile;                 /* Value of --profile; heap-allocated via g_strdup */
    gboolean login_shell;           /* Terminal will spawn login shells */
    gboolean no_remote;             /* Isolated remote messaging signal flag */
} CommandArguments;

/* External API Prototype Declarations */
...
extern void terminal_update_menu_shortcuts(Setting * setting);
extern void terminal_initialize_menu_shortcuts(Setting * setting);
