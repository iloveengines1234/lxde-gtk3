...
static void on_error_dialog_response(GtkWidget *dialog, gint response_id, gpointer user_data);
...
void obconf_error(gchar *msg, gboolean modal)
{
    ...
    d = gtk_message_dialog_new(mainwin ? GTK_WINDOW(mainwin) : NULL,
                               GTK_DIALOG_DESTROY_WITH_PARENT | (modal ? GTK_DIALOG_MODAL : 0),
                               GTK_MESSAGE_ERROR,
                               GTK_BUTTONS_CLOSE,
                               "%s", msg);

    /* Enforce modern transient hierarchy properties across toolbars */
    if (mainwin) {
        gtk_window_set_transient_for(GTK_WINDOW(d), GTK_WINDOW(mainwin));
    }
                               
    gtk_window_set_title(GTK_WINDOW(d), _("ObConf Error"));

    g_signal_connect(d, "response", G_CALLBACK(on_error_dialog_response), NULL);
    
    /* Native single-layer display visibility loop execution */
    gtk_widget_show(d);
}
...
extern gboolean plugin_load(LXAppearance *app, GtkBuilder *lxappearance_builder)
{
    ...
    /* Enforced native X11 windowing backend isolation checks */
    ...
    g_debug("Loading layout structure: %s", GLADEDIR "/obconf.glade");
    ...
    /* Native GTK 3 layout parsing structures */
    gtk_box_pack_start(GTK_BOX(app->wm_page), GTK_WIDGET(gtk_builder_get_object(builder, "obconf_vbox")), TRUE, TRUE, 0);
    gtk_widget_show(app->wm_page);
