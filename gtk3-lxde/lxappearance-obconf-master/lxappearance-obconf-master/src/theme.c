/* -*- indent-tabs-mode: nil; tab-width: 4; c-basic-offset: 4; -*-

   theme.c for ObConf, the configuration tool for Openbox
   Copyright (c) 2003-2007   Dana Jansens
   Copyright (c) 2003        Tim Riley
   Copyright (C) 2010        Hong Jen Yee (PCMan) <pcman.tw@gmail.com>
   Copyright (C) 2026        LXDE Maintainers Group

   This program is free software; you can redistribute it and/or modify
   it under the terms of the GNU General Public License as published by
   the Free Software Foundation; either version 2 of the License, or
   (at your option) any later version.
*/

#include "main.h"
#include "tree.h"
#include "preview_update.h"
#include <glib/gi18n.h>
#include "archive.h"
#include "theme.h"
#include "preview.h"
#include <string.h>

static gboolean mapping = FALSE;
static GList *themes = NULL;
static GtkListStore *theme_store = NULL;

/* Forward Declarations */
static void add_theme_dir(const gchar *dirname);
static void on_theme_names_selection_changed(GtkTreeSelection *sel, gpointer data);
void on_install_theme_clicked(GtkButton *w, gpointer data);
void on_theme_archive_clicked(GtkButton *w, gpointer data);

void theme_setup_tab(void)
{
    GtkCellRenderer *render;
    GtkTreeViewColumn *column;
    GtkTreeSelection *select;
    GtkWidget *w;

    mapping = TRUE;
    w = get_widget("theme_names");
    if (!w) return;

    /* Widget framework allocation initialization */
    theme_store = gtk_list_store_new(1, G_TYPE_STRING);
    gtk_tree_view_set_model(GTK_TREE_VIEW(w), GTK_TREE_MODEL(theme_store));

    preview_update_set_tree_view(GTK_TREE_VIEW(w), theme_store);
    g_object_unref(theme_store);

    select = gtk_tree_view_get_selection(GTK_TREE_VIEW(w));
    gtk_tree_selection_set_mode(select, GTK_SELECTION_SINGLE);

    /* Text render presentation processing layers */
    render = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("Name"), render, "text", 0, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(w), column);

    g_signal_connect(G_OBJECT(select), "changed",
                     G_CALLBACK(on_theme_names_selection_changed), NULL);

    mapping = FALSE;
}

static void on_theme_names_selection_changed(GtkTreeSelection *sel, gpointer data)
{
    GtkTreeIter iter;
    GtkTreeModel *model;
    gchar *name = NULL;

    if (mapping) return;

    if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
        gtk_tree_model_get(model, &iter, 0, &name, -1);
    }

    if (name) {
        tree_set_string("theme/name", name);
        g_free(name);
    }

    preview_update_all();
}

void on_install_theme_clicked(GtkButton *w, gpointer data)
{
    GtkWidget *d;
    gint r;
    gchar *path = NULL;
    GtkFileFilter *filter;

    /* Decoupled from deprecated GTK Stock types */
    d = gtk_file_chooser_dialog_new(_("Choose an Openbox theme"),
                                    GTK_WINDOW(mainwin),
                                    GTK_FILE_CHOOSER_ACTION_OPEN,
                                    _("_Cancel"), GTK_RESPONSE_CANCEL,
                                    _("_Open"), GTK_RESPONSE_OK,
                                    NULL);

    gtk_file_chooser_set_show_hidden(GTK_FILE_CHOOSER(d), FALSE);
    filter = gtk_file_filter_new();
    gtk_file_filter_set_name(filter, _("Openbox theme archives"));
    gtk_file_filter_add_pattern(filter, "*.obt");
    gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(d), filter);

    r = gtk_dialog_run(GTK_DIALOG(d));
    if (r == GTK_RESPONSE_OK) {
        path = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(d));
    }
    gtk_widget_destroy(d);

    if (path != NULL) {
        theme_install(path);
        g_free(path);
    }
}

void on_theme_archive_clicked(GtkButton *w, gpointer data)
{
    GtkWidget *d;
    gint r;
    gchar *path = NULL;

    d = gtk_file_chooser_dialog_new(_("Choose an Openbox theme"),
                                    GTK_WINDOW(mainwin),
                                    GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER,
                                    _("_Cancel"), GTK_RESPONSE_CANCEL,
                                    _("_OK"), GTK_RESPONSE_OK,
                                    NULL);

    gtk_file_chooser_set_show_hidden(GTK_FILE_CHOOSER(d), TRUE);
    r = gtk_dialog_run(GTK_DIALOG(d));
    if (r == GTK_RESPONSE_OK) {
        path = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(d));
    }
    gtk_widget_destroy(d);

    if (path != NULL) {
        archive_create(path);
        g_free(path);
    }
}

void theme_install(const gchar *path)
{
    gchar *name = archive_install(path);
    if (name) {
        tree_set_string("theme/name", name);
        g_free(name);
    }
    theme_load_all();
}

void theme_load_all(void)
{
    gchar *name;
    gchar *p;
    GList *it, *next;
    gint i;
    GtkWidget *w;

    mapping = TRUE;

    w = get_widget("theme_names");
    if (!w) {
        mapping = FALSE;
        return;
    }

    name = tree_get_string("theme/name", "TheBear");

    /* Clear and free existing cached links safely */
    for (it = themes; it; it = g_list_next(it)) {
        g_free(it->data);
    }
    g_list_free(themes);
    themes = NULL;

    /* Build and walk system directory locations */
    p = g_build_filename(g_get_home_dir(), ".themes", NULL);
    add_theme_dir(p);
    g_free(p);

    {
        GSList *sit;
        for (sit = obt_paths_data_dirs(paths); sit; sit = g_slist_next(sit)) {
            p = g_build_filename(sit->data, "themes", NULL);
            add_theme_dir(p);
            g_free(p);
        }
    }

    add_theme_dir(THEMEDIR);

    /* Process collection configurations */
    themes = g_list_reverse(themes);
    themes = g_list_sort(themes, (GCompareFunc) g_ascii_strcasecmp);

    gtk_list_store_clear(theme_store);

    i = 0;
    it = themes;
    while (it != NULL) {
        GtkTreeIter iter;
        next = g_list_next(it);

        /* Safely clear matching duplicate records */
        if (next && strcmp(it->data, next->data) == 0) {
            g_free(it->data);
            themes = g_list_delete_link(themes, it);
            it = next; 
            continue;
        }

        gtk_list_store_append(theme_store, &iter);
        gtk_list_store_set(theme_store, &iter, 0, it->data, -1);

        if (strcmp(name, it->data) == 0) {
            GtkTreePath *tpath = gtk_tree_path_new_from_indices(i, -1);
            if (tpath) {
                gtk_tree_view_set_cursor(GTK_TREE_VIEW(w), tpath, NULL, FALSE);
                gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(w), tpath, NULL, FALSE, 0.0, 0.0);
                gtk_tree_path_free(tpath);
            }
        }
        ++i;
        it = next;
    }

    preview_update_all();
    g_free(name);

    mapping = FALSE;
}

static void add_theme_dir(const gchar *dirname)
{
    GDir *dir;
    const gchar *n;

    if (!dirname) return;

    if ((dir = g_dir_open(dirname, 0, NULL))) {
        while ((n = g_dir_read_name(dir))) {
            gchar *full = g_build_filename(dirname, n, "openbox-3", "themerc", NULL);
            if (!g_file_test(full, G_FILE_TEST_IS_REGULAR | G_FILE_TEST_IS_SYMLINK)) {
                n = NULL;
            }
            g_free(full);

            if (n) {
                themes = g_list_prepend(themes, g_strdup(n));
            }
        }
        g_dir_close(dir);
    }
}