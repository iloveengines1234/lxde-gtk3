...
#include <X11/Xmd.h>    /* For CARD16 / CARD32 */
...
struct _XSettingsManager
{
  ...
  XSettingsList *settings; /* Instance settings list */
 ...
 *
 * Routine to get the current X server time stamp safely.
 * Return value: the time stamp, or 0 on failure timeout.
 **/
static Time
                 ...
                 Window   window)
{
  ...
  /* Modern Fix: Replaced dangerous blocking XIfEvent loop with a timed loop 
   * using XCheckIfEvent to prevent the daemon freezing indefinitely. */
  XFlush(display);
  while (loop_count++ < max_loops)
    {
      ...
      g_usleep(1000); /* Sleep 1ms to allow server processing */
    }
                       ...
                       void                   *cb_data)
{
  ...
  if (XGetSelectionOwner (display, manager->selection_atom) == manager->window)
    {
      ...
      xev.data.l[1] = (long)manager->selection_atom;
...
xsettings_manager_destroy (XSettingsManager *manager)
{
  ...
  if (manager->display && manager->window)
...
{
  return manager ? manager->window : None;
}
                               ...
                               XSettingsSetting *setting)
{
  ...
  if (!manager || !setting || !setting->name)
    return XSETTINGS_FAILED;

  old_setting = xsettings_list_lookup (manager->settings, setting->name);
                           ...
                           int               value)
{
  ...
  /* Safety: Clear memory space completely to neutralize comparison bugs on uninitialized struct space */
...
/* Modern Fix: Safe byte-write serialization functions to entirely bypass 
 * unaligned pointer dereference crashes (SIGBUS bus faults). */
static void
write_card16 (unsigned char **pos, CARD16 val)
{
...
static void
write_card32 (unsigned char **pos, CARD32 val)
{
  memcpy(*pos, &val, sizeof(val));
  *pos += sizeof(val);
}
               ...
               XSettingsBuffer  *buffer)
{
  ...
  *(buffer->pos++) = 0;
  ...
  write_card16 (&buffer->pos, (CARD16)string_len);
  ...
  write_card32 (&buffer->pos, (CARD32)setting->last_change_serial);

  switch (setting->type)
    {
    case XSETTINGS_TYPE_INT:
      write_card32 (&buffer->pos, (CARD32)setting->data.v_int);
      break;
    case XSETTINGS_TYPE_STRING:
      string_len = strlen (setting->data.v_string);
      write_card32 (&buffer->pos, (CARD32)string_len);
      ...
      break;
    case XSETTINGS_TYPE_COLOR:
      write_card16 (&buffer->pos, setting->data.v_color.red);
      write_card16 (&buffer->pos, setting->data.v_color.green);
      write_card16 (&buffer->pos, setting->data.v_color.blue);
      write_card16 (&buffer->pos, setting->data.v_color.alpha);
      break;
    }
}
...
xsettings_manager_notify (XSettingsManager *manager)
{
  ...
  *buffer.pos = xsettings_byte_order ();
  ...
  write_card32 (&buffer.pos, (CARD32)manager->serial++);
  write_card32 (&buffer.pos, (CARD32)n_settings);
