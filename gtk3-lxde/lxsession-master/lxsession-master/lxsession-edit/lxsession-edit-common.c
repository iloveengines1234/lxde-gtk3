...
static void add_autostart_file(gpointer key, gpointer value, gpointer user_data)
{
    char* desktop_id = (char*)key;
    char* file = (char*)value;
    GKeyFile* kf = (GKeyFile*)user_data;
    GtkTreeIter it;
...
void load_autostart(const char* session_name)
{
    ...
    if( g_hash_table_size( hash ) > 0 )
    {
        ...
        g_hash_table_foreach( hash, add_autostart_file, kf );
        g_key_file_free( kf );
    }
...
void update_enable_state(GKeyFile* kf, gboolean enabled, int flags, const char* session_name)
{
        ...
        if( list )
        {
            for( i = 0; i < n; ++i )
            {
                if( strcmp(list[i], session_name) != 0 )
                    g_ptr_array_add(new_list, g_strdup(list[i]));
            }
            g_strfreev(list);
        }

        if( !enabled ) /* add our DE to NotShowIn */
        {
            g_ptr_array_add(new_list, g_strdup(session_name));
        }

        if( new_list->len > 0 )
        {
            g_ptr_array_add(new_list, NULL);
            g_key_file_set_string_list( kf, grp, "NotShowIn", (const gchar * const *) new_list->pdata, new_list->len - 1 );
        }
        else
        {
    ...
    else if( flags & ONLY_SHOW_IN )
    {
        ...
        {
            g_ptr_array_add(new_list, NULL);
            g_key_file_set_string_list(kf, grp, "OnlyShowIn", (const gchar * const *) new_list->pdata, new_list->len - 1 );
        }
        else
        {
...
void init_list_view( GtkTreeView* view )
{
    ...
    autostart_list = gtk_list_store_new(N_COLS,
                                        G_TYPE_BOOLEAN,
                                        G_TYPE_OBJECT, /* Modernized GDK_TYPE_PIXBUF type reference */
                                        G_TYPE_STRING,
                                        G_TYPE_STRING,
                                        G_TYPE_STRING,
                                        G_TYPE_STRING,
