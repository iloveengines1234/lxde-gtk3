 ...
 *
 * Updated by iloveengines1234 to port to gtk3.24.52
 */
...
/* Fix: Compile cleanly against X11 target backends safely inside modern multi-target GDK 3 environments */
                                ...
                                Window   window)
{
    ...
    XEvent xevent;
    int loop_count = 0;
    const int max_loops = 100; /* Prevent UI thread hanging */

    if (!xdisplay || window == None)
        return 0;
    ...
    {
        if (XCheckIfEvent (xdisplay, &xevent, xfce_xsettings_helper_timestamp_predicate, GUINT_TO_POINTER (window)))
        {
            return xevent.xproperty.time;
        }
        g_usleep (500); /* Minimize blocking pressure on GMainLoop */
    }
...
gsd_clipboard_manager_init (GsdClipboardManager *manager)
{
        ...
        if (GDK_IS_X11_DISPLAY (gdk_disp)) {
                manager->priv->display = GDK_DISPLAY_XDISPLAY (gdk_disp);
        }
#endif
}
                           ...
                           XEvent              *xev)
{
        ...
        if (xev->xselectionrequest.target == XA_SAVE_TARGETS) {
                if (manager->priv->requestor != None || manager->priv->contents != NULL) {
                        ...
                        gdk_error_trap_push ();
                        clipboard_manager_watch_cb (manager, xev->xselectionrequest.requestor, True);
                        XSelectInput (manager->priv->display, xev->xselectionrequest.requestor, StructureNotifyMask);
                        XSync (manager->priv->display, False);
                                 ...
                                 XEvent              *xev)
{
        ...
        case DestroyNotify:
                if (xev->xdestroywindow.window == manager->priv->requestor) {
                        ...
                        clipboard_manager_watch_cb (manager, manager->priv->requestor, False);
                        manager->priv->requestor = None;
                }
                break;
        ...
        case SelectionClear:
                ...
                if (xev->xselectionclear.selection == XA_CLIPBOARD_MANAGER) {
                        if (manager->priv->contents) {
                                ...
                                XSetSelectionOwner (manager->priv->display, XA_CLIPBOARD, None, manager->priv->time);
                        }
                        return True;
                }
...
/* Fix: Enforce exact GdkFilterFunc signature declaration to guarantee safety with standard casts */
static GdkFilterReturn
clipboard_manager_event_filter (GdkXEvent *xevent,
                                GdkEvent  *event,
                                gpointer   data)
{
        GsdClipboardManager *manager = GSD_CLIPBOARD_MANAGER (data);
        (void)event;
                            ...
                            gboolean             is_start)
{
        ...
        /* Fix: Rely strictly on modern foreign window tracking rather than dead internal lookup tables */
        if (is_start) {
                gdkwin = gdk_x11_window_foreign_new_for_display (display, window);
                if (gdkwin != NULL) {
                        gdk_window_add_filter (gdkwin, clipboard_manager_event_filter, manager);
                }
        } else {
                gdkwin = gdk_x11_window_lookup_for_display (display, window);
                if (gdkwin != NULL) {
                        gdk_window_remove_filter (gdkwin, clipboard_manager_event_filter, manager);
                }
        }
#endif
}
                             ...
                             gboolean             replace)
{
        ...
        if (!manager->priv->display)
                return FALSE;

        init_atoms (manager->priv->display);

        if (!replace && XGetSelectionOwner (manager->priv->display, XA_CLIPBOARD_MANAGER)) {
                return FALSE;
        }
        ...
        clipboard_manager_watch_cb (manager, manager->priv->window, True);
        XSelectInput (manager->priv->display, manager->priv->window, PropertyChangeMask);
        ...
        if (XGetSelectionOwner (manager->priv->display, XA_CLIPBOARD_MANAGER) == manager->priv->window) {
                ...
                clipboard_manager_watch_cb (manager, manager->priv->window, False);
        }
