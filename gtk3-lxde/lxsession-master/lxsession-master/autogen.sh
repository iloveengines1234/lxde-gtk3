#!/bin/sh
# Updated by iloveengines1234 to support modern autotools and GTK 3.24.52 compilation environments

AUTOMAKE=${AUTOMAKE:-automake}
AM_INSTALLED_VERSION=$($AUTOMAKE --version | sed -e '2,$ d' -e 's/.* \([0-9]*\.[0-9]*\).*/\1/')

# Modern Fix: Expanded regex matching checks to natively accommodate all current 
# Automake release series versions (including 1.10 through 1.17+) without throwing false failures.
