...
static void on_about(GtkAction *act, PluginData *data)
{
    ...
    gtk_about_dialog_set_program_name(about, "LXHotkey");
    gtk_about_dialog_set_logo_icon_name(about, LXHOTKEY_ICON);

    gtk_about_dialog_set_copyright(about, _("Copyright (C) 2016-2026"));
...
static GtkActionEntry actions[] =
{
    ...
    { "New",       GTK_STOCK_NEW, NULL, NULL, N_("Create new action"), G_CALLBACK(on_new) },
    { "Del",       GTK_STOCK_DELETE, NULL, "", N_("Remove selected action"), G_CALLBACK(on_del) },
    { "Edit",      GTK_STOCK_EDIT, NULL, NULL, N_("Change selected action"), G_CALLBACK(on_edit) },
    { "HelpMenu",  NULL, N_("_Help"), NULL, NULL, NULL },
    { "About",     GTK_STOCK_ABOUT, NULL, NULL, NULL, G_CALLBACK(on_about) }
};

static void on_notebook_switch_page(GtkNotebook *nb, GtkTreeView *page, guint num, PluginData *data)
{
...
static void on_row_activated(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *column, PluginData *data)
{
    _edit_action(data, NULL);
}
...
static void module_gtk_run(const gchar *wm, const LXHotkeyPluginInit *cb, gpointer *config, GError **error)
{
    GtkUIManager *ui;
...
#if GTK_CHECK_VERSION(3, 0, 0)
    if (gtk_get_major_version() > 3 || (gtk_get_major_version() == 3 && gtk_get_minor_version() >= 2))
    ...
    g_signal_connect_after(data.notebook, "switch-page", G_CALLBACK(on_notebook_switch_page), &data);
    ...
    gtk_scrolled_window_set_policy(scrwin, GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    ...
    if (cb->get_wm_keys)
    {
        ...
        g_signal_connect(gtk_tree_view_get_selection(data.acts), "changed", G_CALLBACK(on_selection_changed), &data);
        gtk_notebook_append_page(data.notebook, GTK_WIDGET(data.acts), gtk_label_new(_("Actions")));
    }
    ...
    if (cb->get_app_keys)
    {
        ...
        g_signal_connect(gtk_tree_view_get_selection(data.apps), "changed", G_CALLBACK(on_selection_changed), &data);
        gtk_notebook_append_page(data.notebook, GTK_WIDGET(data.apps), gtk_label_new(_("Programs")));
    }
...
void _show_error(const gchar *prefix, GError *error)
{
    GtkWidget *dlg;

    if (error && error->message)
    {
        dlg = gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s%s", prefix, error->message);
