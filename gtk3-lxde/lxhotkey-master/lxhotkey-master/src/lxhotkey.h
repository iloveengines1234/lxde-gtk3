 ...
 *
 * Data descriptor for actions and options. Actions are activated by keybinding.
...
/* FIXED: Marked comparison handlers as G_INLINE_FUNC to stop 'unused-function' 
 * compilation breaks when plugins include this header under strict warning profiles. */
static inline gboolean values_equal(GList *vals1, GList *vals2)
{
    while (vals1 && vals2) {
 ...
 */
static inline gboolean options_equal(GList *opts1, GList *opts2)
{
    while (opts1 && opts2) {
 ...
 * @accel1: a keybinding to activate @actions (Format depends on target backend display server)
 ...
 * Traditional X11 backends use GDK accelerator formats like "<Control>a".
 * Modern native Wayland backends use standard xkbcommon naming sequences (e.g. "Ctrl+a").
 */
typedef struct {
    GList *actions;
    gchar *accel1;
 ...
 * @accel1: a keybinding to activate @exec (Format depends on target backend display server)
...
/* --- WM Backend Modular Core Plugins --- */
 ...
 * @init: callback to bootstrap internal parameters before main execution loop
 *
 * The @run callback receives name of WM, pointer to callbacks, and pointer to
 * pointer to the config data which is already successfully loaded and ready to
