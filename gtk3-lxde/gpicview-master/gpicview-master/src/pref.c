...
void load_preferences()
{
    ...
    /* Fallback default background initialization via standard double floats */
    pref.bg.red = pref.bg.green = pref.bg.blue = 1.0;
    pref.bg.alpha = 1.0;
    pref.bg_full.red = pref.bg_full.green = pref.bg_full.blue = 0.0;
    pref.bg_full.alpha = 1.0;
...
void save_preferences()
{
    ...
    if( (f = fopen( path, "w" )) )
    {
        ...
        char *bg_str = gdk_rgba_to_string(&pref.bg);
        char *bg_full_str = gdk_rgba_to_string(&pref.bg_full);
        fprintf( f, "bg=%s\n", bg_str );
        fprintf( f, "bg_full=%s\n", bg_full_str );
        g_free(bg_str);
        g_free(bg_full_str);
...
/* Internal helper to apply layout CSS transitions cleanly */
static void apply_widget_background(GtkWidget *widget, const GdkRGBA *color)
{
    GtkStyleContext *context = gtk_widget_get_style_context(widget);
    GtkCssProvider *provider = gtk_css_provider_new();
    char *css = g_strdup_printf("widget, box, eventbox { background-color: %s; }", gdk_rgba_to_string(color));
...
static void on_set_bg( GtkColorButton* btn, gpointer user_data )
{
    ...
    {
        apply_widget_background(parent->evt_box, &pref.bg);
...
static void on_set_bg_full( GtkColorButton* btn, gpointer user_data )
{
    ...
    if( parent->full_screen )
    {
        apply_widget_background(parent->evt_box, &pref.bg_full);
        gtk_widget_queue_draw(parent->evt_box);
    }
}
