...
void main_win_finalize( GObject* obj )
{
    ...
    G_OBJECT_CLASS(main_win_parent_class)->finalize(obj);
    gtk_main_quit();
}
...
void main_win_init( MainWin* mw )
{
    ...
    /* Set background color via modern CSS context engine */
    ...
    image_view_set_adjustments( IMAGE_VIEW(mw->img_view), hadj, vadj );
    gtk_container_add( (GtkContainer*)mw->scroll, mw->evt_box );
    ...
    /* Navbar Assembly and Visibility Sync */
    ...
    mw->hand_cursor = gdk_cursor_new_from_name(gtk_widget_get_display((GtkWidget*)mw), "all-scroll");
...
void create_nav_bar( MainWin* mw, GtkWidget* box )
{
    ...
    mw->btn_fit = add_nav_btn( mw, "zoom-fit-best", _("Fit Image To Window Size"), G_CALLBACK(on_zoom_fit), TRUE );
    mw->btn_orig = add_nav_btn( mw, "zoom-original", _("Original Size"), G_CALLBACK(on_orig_size), TRUE );
    gtk_toggle_button_set_active( (GtkToggleButton*)mw->btn_fit, TRUE );

    add_nav_btn( mw, "view-fullscreen", _("Full Screen"), G_CALLBACK(on_full_screen), FALSE );
    ...
    /* Center navigation alignment block */
...
gboolean main_win_open( MainWin* mw, const char* file_path, ZoomMode zoom )
{
    ...
    /* Grabs target file stream as an animated canvas */
    ...
    /* Viewport Constraint Evaluations */
    if( zoom == ZOOM_NONE )
    {
        ...
        if( w + pref.win_x < area.width && h + pref.win_y < area.height && (w >= pref.win_w || h >= pref.win_h) )
        {
            ...
            GtkRequisition req;
            gtk_widget_get_preferred_size( (GtkWidget*)mw, NULL, &req );
    ...
    if( gtk_widget_get_realized(widget) )
    {
        ...
        if( mw->zoom_mode == ZOOM_FIT )
        {
            while( gtk_events_pending() )
                gtk_main_iteration(); /* Fluid rendering layout trick */
...
void main_win_fit_size( MainWin* mw, int width, int height, gboolean can_strech, GdkInterpType type )
{
    ...
    else    /* Fall back onto original dimensional layout if smaller */
    {
        mw->scale = 1.0;
...
void on_save( GtkWidget* btn, MainWin* mw )
{
    ...
    GdkPixbufFormat* info = gdk_pixbuf_get_file_info( file_name, NULL, NULL );
...
gboolean on_button_press( GtkWidget* widget, GdkEventButton* evt, MainWin* mw )
{
    ...
    {
        if( evt->button == 1 )  /* left click dragging */
        {
            if( ! mw->pix )
                return FALSE;
            mw->dragging = TRUE;
            ...
            gdk_window_get_device_position(window, device, &mw->drag_old_x, &mw->drag_old_y, NULL);

            gdk_window_set_cursor( window, mw->hand_cursor );
        }
        else if( evt->button == 3 )   /* right click menu */
        {
            show_popup_menu( mw, evt );
        }
    }
    else if( evt->type == GDK_2BUTTON_PRESS && evt->button == 1 )  /* full screen toggle */
    {
...
gboolean on_mouse_move( GtkWidget* widget, GdkEventMotion* evt, MainWin* mw )
{
    ...
    GdkSeat *seat = gdk_display_get_default_seat(display);
    GdkDevice *device = gdk_seat_get_pointer(seat);
    gdk_window_get_device_position(window, device, &cur_x, &cur_y, NULL);

    int dx = (mw->drag_old_x - cur_x);
    int dy = (mw->drag_old_y - cur_y);

    GtkAdjustment *hadj = gtk_scrolled_window_get_hadjustment((GtkScrolledWindow*)mw->scroll);
    GtkAdjustment *vadj = gtk_scrolled_window_get_vadjustment((GtkScrolledWindow*)mw->scroll);
...
gboolean main_win_save( MainWin* mw, const char* file_path, const char* type, gboolean confirm )
{
    ...
    for (gdk_formats_i = gdk_formats; gdk_formats_i; gdk_formats_i = g_slist_next(gdk_formats_i))
    {
...
void show_popup_menu( MainWin* mw, GdkEventButton* evt )
{
    ...
    {
        PTK_IMG_MENU_ITEM( N_( "Previous" ), "go-previous", on_prev, GDK_KEY_Left, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Next" ), "go-next", on_next, GDK_KEY_Right, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Start/Stop Slideshow" ), "media-playback-start", on_slideshow_menu, GDK_KEY_w, 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Zoom Out" ), "zoom-out", on_zoom_out, GDK_KEY_minus, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Zoom In" ), "zoom-in", on_zoom_in, GDK_KEY_plus, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Fit Image To Window Size" ), "zoom-fit-best", on_zoom_fit_menu, GDK_KEY_f, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Original Size" ), "zoom-original", on_orig_size_menu, GDK_KEY_g, 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Full Screen" ), "view-fullscreen", on_full_screen, GDK_KEY_F11, 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Rotate Counterclockwise" ), "object-rotate-left", on_rotate_counterclockwise, GDK_KEY_l, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Rotate Clockwise" ), "object-rotate-right", on_rotate_clockwise, GDK_KEY_r, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Flip Horizontal" ), "object-flip-horizontal", on_flip_horizontal, GDK_KEY_h, 0 ),
        PTK_IMG_MENU_ITEM( N_( "Flip Vertical" ), "object-flip-vertical", on_flip_vertical, GDK_KEY_v, 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Open File"), "document-open", G_CALLBACK(on_open), GDK_KEY_o, 0 ),
        PTK_IMG_MENU_ITEM( N_("Save File"), "document-save", G_CALLBACK(on_save), GDK_KEY_s, 0 ),
        PTK_IMG_MENU_ITEM( N_("Save As"), "document-save-as", G_CALLBACK(on_save_as), GDK_KEY_a, 0 ),
        PTK_IMG_MENU_ITEM( N_("Delete File"), "edit-delete", G_CALLBACK(on_delete), GDK_KEY_Delete, 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Preferences"), "preferences-system", G_CALLBACK(on_preference), GDK_KEY_p, 0 ),
        PTK_IMG_MENU_ITEM( N_("Show/Hide toolbar"), NULL, G_CALLBACK(on_toggle_toolbar), GDK_KEY_t, 0 ),
        PTK_STOCK_MENU_ITEM( "help-about", on_about ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Quit"), "application-exit", G_CALLBACK(on_quit), GDK_KEY_q, 0 ),
        PTK_MENU_END
    };
    GtkWidget* rotate_cw;
    ...
    /* Modern context placement replacing deprecated manual coordinate injections */
...
void on_about( GtkWidget* menu, MainWin* mw )
{
    ...
    gtk_about_dialog_set_copyright ( (GtkAboutDialog*)about_dlg, _( "Copyright (C) 2007 - 2026" ) );
