...
char* get_open_filename(GtkWindow* parent, const char* cwd)
{
    ...
    // Updated to GTK 3 native button text strings instead of GTK_STOCK constants
...
static void on_file_save_filter_changed(GObject* obj, GParamSpec* pspec, gpointer user_data)
{
    ...
    if (type)
    {
        ...
        int* pref_val = NULL;
        double min_val = 0, max_val = 0, step_val = 1;
        ...
        if (pref_val != NULL)
        {
            // Pure GTK 3 horizontal box structure
            extra = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
            scale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, min_val, max_val, step_val);
            gtk_range_set_value(GTK_RANGE(scale), *pref_val);

            g_signal_connect(G_OBJECT(scale), "value-changed", G_CALLBACK(on_int_val_changed), pref_val);
...
char* get_save_filename(GtkWindow* parent, const char* cwd, char** type)
{
    ...
    if (gtk_dialog_run((GtkDialog*)dlg) == GTK_RESPONSE_OK)
    {
        ...
        *type = g_object_steal_data(G_OBJECT(filter), "type");
        ...
        if (!*type)
        {
            /* TODO: Implement auto file type detection logic here */
        }
        else
        {
            /* TODO: Append the appropriate extensions if missing */
        }
    }
    gtk_widget_destroy((GtkWidget*)dlg);
