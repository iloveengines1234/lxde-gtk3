...
static int SectionsAllocated = 0;
static int SectionsRead = 0;
static int HaveAll = 0;

void CheckSectionsAllocated(void);

/* Removed trailing semicolon to prevent compilation errors during assignment */
...
static void process_COM (const uchar * Data, int length)
{
    ...
    /* Standardized to safe strncpy boundary tracking */
    strncpy(ImageInfo.Comments, Comment, MAX_COMMENT_SIZE);
    ImageInfo.Comments[MAX_COMMENT_SIZE] = '\0';
}
...
void CheckSectionsAllocated(void)
{
    ...
    if (SectionsRead >= SectionsAllocated){
        /* Standardized boundary expansion to prevent math issues with odd values */
        SectionsAllocated = (SectionsAllocated < 10) ? 16 : SectionsAllocated * 2;
            ...
            case M_EXIF:
                if (ReadMode & READ_METADATA){
                    if (memcmp(Data+2, "Exif", 4) == 0){
                        ...
                        Sections[SectionsRead-1].Type = M_XMP; 
                        if (ShowTags){
...
/* Activated missing pipeline utilities */
...
int SaveThumbnail(char * ThumbFileName)
{
    ...
    if (ThumbnailFile){
        ...
        if (!ExifSection) {
            if (ThumbnailFile != stdout) fclose(ThumbnailFile);
            return FALSE;
        }
...
int ReplaceThumbnail(const char * ThumbFileName)
{
    ...
    ExifSection = FindSection(M_EXIF);
    if (!ExifSection) {
        if (ThumbnailFile) fclose(ThumbnailFile);
        return FALSE;
    }
    ...
    if (ThumbnailFile){
        size_t read_bytes = fread(ThumbnailPointer, 1, ThumbLen, ThumbnailFile);
        (void)read_bytes;
        fclose(ThumbnailFile);
    }
...
void DiscardAllButExif(void)
{
    ...
    memset(&XmpKeeper, 0, sizeof(XmpKeeper));
...
//--------------------------------------------------------------------------
// Add a section (assume it doesn't already exist)
//--------------------------------------------------------------------------
Section_t * CreateSection(int SectionType, unsigned char * Data, int Size)
{
    ...
    if (SectionType == M_EXIF) NewIndex = 0; 
...
{
    if (Sections == NULL){
        SectionsAllocated = 16;
        Sections = (Section_t *)malloc(sizeof(Section_t) * SectionsAllocated);
    }
    SectionsRead = 0;
    HaveAll = 0;
}