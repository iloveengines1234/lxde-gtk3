...
#include <string.h>
#include <sys/stat.h> // Ensure stat struct is recognized cleanly across modern systems
...
ImageList* image_list_new()
{
    ...
    if( ! supported_formats )
    {
        ...
        for( format = formats; format; format = format->next )
        {
            ...
            g_free( exts ); // g_strfreev is not needed since we stole its strings
        }
    }
...
// Cleaned up and standardized for safe UTF-8 collating in GTK3
static int comp_by_name( char* name1, char* name2, GtkSortType type )
{
    char* utf8_1 = g_filename_display_name(name1);
    char* casefold_1 = g_utf8_casefold( utf8_1, -1 );
    g_free( utf8_1 );

    char* utf8_2 = g_filename_display_name(name2);
    char* casefold_2 = g_utf8_casefold( utf8_2, -1 );
    g_free( utf8_2 );

    int ret = g_utf8_collate( casefold_1, casefold_2 );
    g_free( casefold_1 );
    g_free( casefold_2 );

    // GTK3 updates sorting flips explicitly based on GtkSortType enum mapping
    return type == GTK_SORT_ASCENDING ? ret : -ret;
}

void image_list_sort_by_name(  ImageList* il, GtkSortType type )
{
    il->list = g_list_sort_with_data( il->list, (GCompareDataFunc)comp_by_name, GINT_TO_POINTER(type) );
}
...
{
    il->list = g_list_insert_sorted_with_data( il->list, g_strdup(name), (GCompareDataFunc)comp_by_name, GINT_TO_POINTER(GTK_SORT_ASCENDING) );
}