                                   ...
                                   GtkAccelGroup* accel_group )
{
  ...
  for( ent = entries; ; ++ent )
  {
    if( G_LIKELY( ent->label ) )
    {
      ...
      /* Handle items that traditionally required an image icon */
      if( G_UNLIKELY( PTK_IS_STOCK_ITEM(ent) ) || G_LIKELY(ent->stock_icon && ent->stock_icon > (char *)2) )  {
        menu_item = gtk_menu_item_new();
        box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
        label = gtk_label_new_with_mnemonic(_(ent->label));
        
        /* Fallback mapping fallback names if still referencing legacy string identifiers */
        const char* icon_name = ent->stock_icon;
        if (!icon_name || icon_name <= (char *)2) {
            icon_name = ent->label; 
        }
