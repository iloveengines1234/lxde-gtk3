...
int rotate_and_save_jpeg_lossless(char *filename, int angle)
{
    ...
    /* Normalize angle tracking to clean absolute ranges */
    angle = angle % 360;
    if (angle < 0)
        angle += 360;
    ...
    if (angle == 90)
        ...
        return EINVAL; /* Catch unhandled arbitrary degree adjustments safely */

    /* Securely generate the temporary layout using GLib's string builder */
    tmpfilename = g_strdup_printf("%s.XXXXXX", filename);
    if (tmpfilename == NULL)
        return ENOMEM;

    /* Create temporary file securely */
    tmpfilefd = g_mkstemp(tmpfilename);
    if (tmpfilefd == -1) {
        g_free(tmpfilename);
        return errno;
    }
    ...
    if (error) {
        ...
        glong_unlink(tmpfilename); /* Safely clear dangling descriptor */
        g_free(tmpfilename);
        return saved_errno;
    }

    /* Rename temporary file over the original file atomically using GLib */
    int error_1 = g_rename(tmpfilename, filename);
    if (error_1 == -1) {
        int saved_errno = errno;
        glong_unlink(tmpfilename);
        g_free(tmpfilename);
        return saved_errno;
    }
