...
static void on_color_set(GtkColorButton* btn, const char* color_name)
{
    ...
    color_str = gdk_rgba_to_string(&clr);
...
static void update_color_buttons()
{
    ...
    for(i = 0; i < 8; ++i)
    {
        ...
        {
            GdkRGBA clr;
            if(gdk_rgba_parse(&clr, color_str))
            {
...
void color_scheme_update()
{
    ...
    if(app.widget_theme)
    {
        ...
        if(!file_found)
        {
            /* GTK3 replacement for deprecated gtk_rc_get_theme_dir system directories search */
            const gchar * const *dirs = g_get_system_data_dirs();
            int i;
            for (i = 0; dirs[i] != NULL; ++i)
            {
                gtkrc = g_build_filename(dirs[i], "themes", app.widget_theme, "gtk-2.0/gtkrc", NULL);
                file_found = gtkrc_file_get_color_scheme(gtkrc, app.default_color_scheme_hash);
                g_free(gtkrc);
                if (file_found)
                    break;
    ...
    if(app.color_scheme_supported && app.use_lxsession)
    {
        ...
        /* if customized color scheme is not used, use default colors of the theme. */
        if(!app.color_scheme)
        {
