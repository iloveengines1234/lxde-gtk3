...
/* Reference external global declaration context */
...
{
    gint res;
    /* Replaced deprecated Stock items with clear modern string properties */
...
_failed:
    ...
    {
        g_debug("pid = %d", pid);
        if(show_progress_for_pid(GTK_WINDOW(app.dlg), _("Install themes"), _("Installing..."), pid))
        {
            GDir* dir;
            ...
            if(dir)
            {
                ...
                /* Migrated from system shell calls to clean, safe asynchronous command generation */
                char* cleanup_cmd = g_strdup_printf("rm -rf '%s'", tmp_dir);
                g_spawn_command_line_async(cleanup_cmd, NULL);
                g_free(cleanup_cmd);
            }
        }
    }
...
gboolean install_icon_theme(GtkWindow* parent)
{
    ...
    /* Removed deprecated Stock triggers and replaced with pure text elements */
    GtkWidget* fc = gtk_file_chooser_dialog_new(_("Select an icon theme"), parent,
...
gboolean remove_icon_theme(GtkWindow* parent, IconTheme* theme)
{
    ...
    if(g_mkdtemp(tmp_dir))
    {
        ...
        if(g_rename(dir, tmp_dest) == 0)
        {
            ...
            if(g_spawn_async(NULL, (char**)argv, NULL, G_SPAWN_SEARCH_PATH|G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, NULL))
            {
                ret = show_progress_for_pid(GTK_WINDOW(app.dlg), _("Remove icon theme"), _("Removing..."), pid);
            }
        }
        g_free(tmp_dest);
    }
