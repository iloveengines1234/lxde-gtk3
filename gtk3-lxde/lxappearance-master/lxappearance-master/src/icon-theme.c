...
static void load_icon_themes(void)
{
    int n, i;
...
{
    /* GTK3 replacement for grabbing target window context from generic layout buttons safely */
    GtkWidget* toplevel = gtk_widget_get_ancestor(GTK_WIDGET(btn), GTK_TYPE_WINDOW);
    if (toplevel != NULL)
    {
        install_icon_theme(GTK_WINDOW(toplevel));
    }
}
...
static void on_remove_theme_clicked(GtkButton* btn, gpointer user_data)
{
    ...
    if(gtk_tree_selection_get_selected(sel, &model, &it))
    {
        ...
        gtk_tree_model_get(model, &it, 1, &theme, -1);
        GtkWidget* toplevel = gtk_widget_get_ancestor(GTK_WIDGET(btn), GTK_TYPE_WINDOW);
        if(toplevel != NULL && remove_icon_theme(GTK_WINDOW(toplevel), theme))
        {
...
void icon_theme_init(GtkBuilder* b)
{
    GSList* l;
    GtkTreeIter it;
    GtkTreeIter icon_theme_sel_it;
    GtkTreeIter cursor_theme_sel_it;
    GtkTreeSelection* sel;
    GtkWidget* btn;
    
    /* Initialize memory containers cleanly to safely evaluate missing tags */
    g_memset(&icon_theme_sel_it, 0, sizeof(GtkTreeIter));
    g_memset(&cursor_theme_sel_it, 0, sizeof(GtkTreeIter));
